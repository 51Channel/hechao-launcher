...
API `0.20.2-20260727T225819Z` 已完成一致性备份、哈希校验、原子切换、客户端会话来源与兼容保护、公网回归和生产矩阵 `8/8`；账号安全、论坛 Cookie 联动、客户端三通道、隐私受限遥测、服务器运行指标和统一告警均在线。Nginx 五个站点入口已启用无查询参数、无 Referer 的访问日志，合成重置 token 回归泄漏数为 `0`。状态采集器 `0.2.0` 与三类指标代理已实时上报大厅、Survival1、Survival2、Activity 和恐怖整蛊（历史目标 `pvp`）的进程、磁盘、TPS、MSPT 与累计 GC；当前仅完成单用户空载基线，不替代多人负载验收。大厅 LuckPerms 等级代理已加载并读回真实 `owner` 映射。生产 Velocity 授权插件 `0.3.0` 已加载为 `monitor`；Minecraft 版本或模组档案不兼容会始终硬拒绝，其他权限拒绝仍只记录。当前开发分支测试为 `.NET 370/370`、Velocity `20/20`、等级代理 `4/4`、指标代理 `2/2`。

真实管理员已完成 MFA 登记，`0.11.14` 已产生首条真实启动遥测，诊断上传、管理员下载、审计和本地 SHA-256 复验均已完成。基础客户端的 Lobby、Survival1、Survival2、Activity 与恐怖整蛊单账号首次路由均已通过；恐怖整蛊的 CrossStitch 修复、身份转发、直连拒绝、稳定连接和正常退出也已验收。跨版本回程开关已在 API `0.21.0` 候选中以默认关闭方式实现，并已在生产数据库副本上通过迁移默认值、目标定向和模组档案防绕过验收。旧隔离 Velocity `3.4.0-SNAPSHOT` 与 `3.5.1` 的真实 `/hub` 分别因 `accept_teleportation` 多出 `131` / `16` 字节断开；Velocity 4 的后端单层 Via 虽产生一条稳定超过 `591` 秒的成功样本，第二次 `/hub` 又因多余 `17` 字节失败。最终隔离结构固定为 Velocity `4.0.0` build `6`、独立 Temurin Java `25.0.4+7`、代理 Via `2`、独立 Lobby Via `0`。真实正版客户端连续五轮完成 fresh grant、恐怖整蛊登录、`/hub`、隔离大厅登录和正常退出；代理、两个后端、客户端及候选 API 协议错误均为 `0`。生产代理和游戏服尚未改动。会话来源修复 Authorizer `0.3.1` 仍只在隔离代理加载；下一步是 API `0.21.0`、Authorizer `0.3.1` 与代理单层 Via 的可回滚生产发布，再做生产真实回程。四级账号、NPC/命令转服和多人灰度仍待验收，因此 `Authentication__EnforceCatalogAuthentication=false` 与生产 Velocity `monitor` 暂时保持不变。

...
7. 启动器 `0.11.10` 正在替换 `0.11.9` 进入私有 OSS 灰度；除真实游戏启动外，重点复验运行配置侧栏在常用窗口尺寸下的 Java 与内存选择器。

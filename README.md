...
> `2026-09-04` 活动企划“先建活动、后加客户端”已随 API `0.38.3` 正式上线。管理员可以
> 先保存未绑定客户端的草稿，随后再选择已完成整合包；未绑定草稿不能发布或部署，也不会
> 进入玩家启动器目录。生产迁移、完整流程、旧版兼容和回滚验收见
> [`docs/API_RELEASE_0.38.3.md`](docs/API_RELEASE_0.38.3.md)。

> `2026-09-04` 活动企划“先建活动、后加客户端”与真实承载服务器绑定已随 API
> `0.38.4` 正式上线。管理员可以先保存未绑定客户端的草稿，随后选择整合包和负责运行它的
> 固定或独立槽；同一承载槽的已发布排期互斥，不同独立槽可并行。逻辑企划不再混入普通
> 服务器目录，企划发布后玩家目录只显示企划卡，不重复显示其真实承载槽。运行状态、部署
> 和 Velocity 授权都使用真实承载槽。未绑定草稿仍不能发布或部署，也不会进入玩家目录。
> 发布记录见 [`docs/API_RELEASE_0.38.4.md`](docs/API_RELEASE_0.38.4.md)。
...
活动预下载与进服门禁分离、签名客户端分发、按哈希增量更新、平滑并行断点续传、SHA-256
校验、修复、主动回滚、原子版本切换、每档案独立 `.minecraft`、共享下载对象、每档案受管
Java 与自定义 Java、Windows 安装包、真实 Minecraft 启动、本地脱敏诊断及玩家确认上传、
隐私受限运行遥测、Velocity 服务端二次授权、只读实时状态与进程指标采集、统一告警，以及带
独立浏览器会话、双重验证、活动排期、经济监控与单品 K 线、玩家搜索、单服权限规则、论坛
会话联动、账号安全操作、整合包导入和受控服控的 Vue 管理控制台。
...
- 使用 15 分钟访问令牌和可撤销、轮换的刷新令牌；完整短期会话由 Windows DPAPI 保护，
  会话文件采用原子替换并保留加密备份。启动时优先复用仍有效的访问令牌；API 暂时断网、
  超时或 5xx 时保留本地登录态并自动等待后续刷新，只有服务端明确返回 401 才清除会话。

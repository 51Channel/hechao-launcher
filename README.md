...
> `2026-08-09` 启动器 `0.15.2` 正在发布：主页移除下方客户端详情区，把回滚、修复、
> 删除、设置和 Java 模式集中到主按钮右侧的三点菜单；快捷设置改为两行当前档案控制栏，
> 游戏目录显示并打开真实的 `instances\<profile-id>\.minecraft`，全局数据根目录只在
> 设置页修改。功能候选的 Release 构建为 `0` 警告、`0` 错误，完整解决方案
> `708/708`，`1500 x 860` 与 `1060 x 640` 真实 WPF 截图通过。正式生产版本在发布
> 切换完成前仍为 `0.15.1`；候选门槛见
> [`docs/LAUNCHER_RELEASE_0.15.2_CANDIDATE.md`](docs/LAUNCHER_RELEASE_0.15.2_CANDIDATE.md)。

...
真实管理员已完成 MFA 登记，`0.11.14` 已产生首条真实启动遥测，诊断上传、管理员下载、审计和本地 SHA-256 复验均已完成。基础客户端的 Lobby、Survival1、Survival2、Activity 与恐怖整蛊单账号首次路由均已通过；恐怖整蛊的 CrossStitch 修复、身份转发、直连拒绝、稳定连接和正常退出也已验收。跨版本回程开关已在 API `0.21.0` 候选中以默认关闭方式实现，并已在生产数据库副本上通过迁移默认值、目标定向和模组档案防绕过验收。首次真实 `/hub` 已通过命令和授权连接大厅，但旧隔离 Velocity `3.4.0-SNAPSHOT` 在切服时发生协议状态解码错误；Velocity `3.5.1` build `615` 实机复测仍因 `accept_teleportation` 多出 `16` 字节断开。隔离核心现已进一步升级为稳定版 Velocity `4.0.0` build `6`，使用独立 Temurin Java `25.0.4+7`。随后发现隔离代理与大厅后端重复加载同一组 ViaVersion/ViaBackwards；现已让隔离代理保持 Via 禁用，由大厅后端承担唯一转换层。代理三个插件、代理 Via `0`、大厅 Via `2` 及 1.13/1.20.1/1.21.11 status 协商均通过，生产代理和游戏服未改动。会话来源修复 Authorizer `0.3.1` 仍只在隔离代理加载；Velocity 4 单层翻译候选的真实 `/hub`、成功重连、四级账号、NPC/命令转服和多人灰度仍待验收，因此 `Authentication__EnforceCatalogAuthentication=false` 与生产 Velocity `monitor` 暂时保持不变。

...
API `0.20.2-20260727T225819Z` 已完成一致性备份、哈希校验、原子切换、客户端会话来源与兼容保护、公网回归和生产矩阵 `8/8`；账号安全、论坛 Cookie 联动、客户端三通道、隐私受限遥测、服务器运行指标和统一告警均在线。Nginx 五个站点入口已启用无查询参数、无 Referer 的访问日志，合成重置 token 回归泄漏数为 `0`。状态采集器 `0.2.0` 与三类指标代理已实时上报大厅、Survival1、Survival2、Activity 和 PVP 的进程、磁盘、TPS、MSPT 与累计 GC；当前仅完成单用户空载基线，不替代多人负载验收。大厅 LuckPerms 等级代理已加载并读回真实 `owner` 映射。生产 Velocity 授权插件 `0.3.0` 已加载为 `monitor`；Minecraft 版本或模组档案不兼容会始终硬拒绝，其他权限拒绝仍只记录。当前开发分支测试为 `.NET 369/369`、Velocity `20/20`、等级代理 `4/4`、指标代理 `2/2`。真实管理员已完成 MFA 登记，`0.11.14` 已产生首条真实启动遥测，诊断上传、管理员下载、审计和本地 SHA-256 复验均已完成。基础客户端的 Lobby、Survival1、Survival2、Activity 与 PVP 单账号首次路由均已通过；PVP 的 CrossStitch 修复、身份转发、直连拒绝、稳定连接和正常退出也已验收。跨版本回程开关已在 API `0.21.0` 候选中以默认关闭方式实现，并已在生产数据库副本上通过迁移默认值、目标定向和模组档案防绕过验收；回环隔离代理也已确认 1.13/1.20.1/1.21.11 status 协议协商，受限凭据 API 探针返回预期 `PlayerNotLinked`，Via 错误为 `0`。会话来源修复 Authorizer `0.3.1` 已仅在隔离代理加载；API 与 Authorizer 候选均尚未部署生产，仍待真实登录、`/hub`、成功重连、四级账号、NPC/命令转服和多人灰度，因此 `Authentication__EnforceCatalogAuthentication=false` 与生产 Velocity `monitor` 暂时保持不变。三份 Paper 世界正式归档、远端 ZIP/旁车复核、异机完整解压、`level.dat` 校验和确定性区域抽样恢复已通过。RAM v5 已默认生效；启动器数据库、论坛与 Sub2API 的异地加密链均已完成真实 OSS 往返、定时任务、告警恢复与异地主机隔离恢复，平台监控器 `0.1.2` 已生产运行。客户端不会使用第三方启动器凭据，不采集 Microsoft 密码，也不保存赫朝账号密码。

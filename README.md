...
> `2026-08-09` 启动器 `0.15.4` 正在发布：主页继续按照确认参考图收口整页布局，除三栏、
> 主卡片、公告和快捷设置外，顶部栏、服务器目录、紧凑导航与底部账户面板也统一到同一
> 组基线和密度。服务器目录、客户端安装、自更新、账号、活动月历和游戏启动状态机保持
> 不变。生产更新通道在最终截图、安装与分发验收前仍为 `0.15.3`；候选门槛见

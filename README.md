...
7. Minecraft API 许可与真实分组账号验收通过后，再向玩家发布启动器 `0.8.0`；五个工作区、下载历史、设置持久化和短屏响应式布局的本机候选检查已通过。

...
API `0.11.1-20260725T165050Z` 已完成部署前备份、哈希校验、原子切换和公网回归；旧官网与中转 API 均保持 200，管理员入口保持既有 404，journal 无新增 warning。启动器 `0.11.6` 已通过 `193/193` 自动测试、本机覆盖升级和 1500 x 860 实机界面验收；登录与注册选择器采用与内存选项相同的等宽完整矩形，两个表单保持整栏拉伸。最终安装包仍只存在于本机构建目录，未上传 OSS 或向玩家公开分发。Velocity 保持既有模式，本次开发没有启动、停止或重启任何 Minecraft、Velocity、大厅、生存服或活动服。客户端不会使用第三方启动器凭据，不采集 Microsoft 密码，也不保存赫朝账号密码。

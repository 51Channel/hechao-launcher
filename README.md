...
6. [已完成] 部署 API `0.20.0`、统一运行告警与状态采集器 `0.2.0`；赫朝账号、对象分发、下载专用限流、授权定向路由、诊断上传、服务器排期、单服访问规则、论坛会话联动、受控全局等级、运行遥测和服务器进程/磁盘指标已上线。真实管理员 MFA 已登记；诊断管理员下载审计、大厅等级代理和三个指标代理仍需分别完成真实验收。

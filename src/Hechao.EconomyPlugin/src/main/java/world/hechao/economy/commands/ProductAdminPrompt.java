...
final class ProductAdminPrompt {
    ...
    static void send(Player player, String itemId) {
        ...
        player.sendMessage(Component.text("更多操作: ", NamedTextColor.GRAY)
                .append(actionButton(
                        "[自定义价格/额度]",
                        ClickEvent.suggestCommand("/heco product set "),
                        "补全单价，可继续填写个人日限和全服日限",
                        NamedTextColor.AQUA))
                .append(Component.space())
                .append(actionButton(
                        "[暂停回收]",
                        ClickEvent.runCommand("/heco product remove"),
                        "立即暂停该物品回收",
                        NamedTextColor.RED)));
    }
    ...
    static List<String> presetCommands() {
        return PRESET_PRICES.stream()
                .map(price -> "/heco product set " + price)
                .toList();
    }
    ...
    private static Component priceButton(String price) {
        return actionButton(
                "[" + price + "]",
                ClickEvent.runCommand("/heco product set " + price),
                "以 " + price + " 金币启用回收",

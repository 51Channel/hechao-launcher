...
创建服务端目录前先阅读
`docs/HECHAO_NEW_SERVER_BASELINE.md`，并从
`docs/examples/server-baseline/component-plan.example.json` 建立组件计划：

| 层级 | 当前组件或能力 | 新活动的动作 |
| --- | --- | --- |
| Velocity 单例 | `HechaoVelocityAuthorizer` | 核验现有代理，不复制进后端 |
| 内部大厅 | Lobby Guard、LuckPerms Tier Agent | 明确排除，不复制进活动服 |
| VPS 主机 | ServerControlAgent、StatusCollector、备份、告警 | 注册目标，不放入 `plugins/mods` |
| 游戏后端 | forwarding、加载器对应指标代理 | 精确版本审查后各保留一个实现 |
| 活动自有 | 玩法 JAR 和已批准依赖 | 按需求单白名单部署 |

当前已有 Paper/Purpur、Fabric `1.20.1` 和 NeoForge `1.21.11` 三种指标实现，但它们不
提供身份 forwarding。Paper/Purpur 使用原生 modern forwarding；Fabric、Forge、
NeoForge 必须按实际版本盘点和批准适配器。缺少兼容实现是生产阻塞，不是复制相似 JAR
的理由。
...
1. 经审查的组件计划，没有继承旧服完整插件、模组或配置目录；
2. 独立 `serverId/controlTargetId`、目录和计划任务；
3. `port=25568`、`conflictGroup=owl5-activity-slot`；
4. 状态采集、TPS/MSPT/GC、CPU、内存、磁盘和告警；
5. 世界备份与恢复目标；
6. 受限控制台命令和明确内存参数文件；
7. 后端只接受已批准的 Velocity forwarding，不开放公网直连。
...
| 要理解的能力 | 先读文件 | 重点 |
| --- | --- | --- |
| 新服务端基础组件 | `docs/HECHAO_NEW_SERVER_BASELINE.md`、`docs/examples/server-baseline/component-plan.example.json` | 部署层、兼容矩阵、组件计划和禁止继承项 |
...
| Velocity 授权单例 | `src/Hechao.VelocityAuthorizer`、`deploy/windows/velocity-authorizer` | 插件只装代理，后端不复制 Authorizer 或内部令牌 |
| 内部大厅组件 | `src/Hechao.LobbyGuard`、`src/Hechao.LuckPermsTierAgent` | 玩家拒绝和等级执行为何不属于活动后端 |
| 后端深度指标 | `src/Hechao.ServerMetricsAgent`、`src/Hechao.ModServerMetricsAgent` | Paper/Fabric/NeoForge 实现边界，指标不提供 forwarding |
| 主机只读状态 | `src/Hechao.StatusCollector`、`tests/Hechao.StatusCollector.Tests` | 通过主机目标读取状态与指标，不把采集器装进服务端 |
| 主机部署参考 | `deploy/windows/server-heartbeats`、`server-metrics`、`mod-server-metrics`、`world-backup`、`pvp-velocity` | 无秘密注册、安装、备份和 Fabric forwarding 参考 |

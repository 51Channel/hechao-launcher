...
> 当前加载版本：`HechaoLuckPermsTierAgent 0.1.3`，等级命令协议 `2`
>
> 当前 API：`0.30.4-20260814T093000Z`，继承等级命令协议 `2` 门禁
>
> 生产状态：真实业务变更证明 `0.1.2` 部署验收遗漏了并行运行的旧 `0.1.0` 实例。
> 遗留实例已隔离，`0.1.3` 与 API `0.30.3` 已正式部署；当前 API `0.30.4` 继续继承
> 该门禁，两条正常业务变更跨四个五分钟间隔未回退
>
...
2026-08-14 `0.1.3` 正式部署结果：

```text
E:\LobbyServer\plugins\HechaoLuckPermsTierAgent-0.1.3.jar
SHA-256 E0637E0AA0A549C5DBDD4FFB3E34238645E6AEA0FDB8BE3780C5822CDBC0700F
回滚目录 E:\manual-backups\luckperms-tier-agent-20260814T080510Z
```

正式大厅 PID 为 `8028`，日志确认 `version 0.1.3, protocol 2` 并到达 `Done`。精确旧版
载荷返回 `400`，协议 `2` 空领取返回 `200`。遗留 `Hechao-Lobby-PvpReturn-Staging`
任务保持 `Disabled`，`25580` 无监听和连接。部署时其他 Java PID `2576 / 7748 / 9428`
未变化；最终复核时 `2576` 已在本任务之外退出，未为恢复旧 PID 集合启动其他服务器。
...
2026-08-14 08:42 UTC，管理员正常提交的两条真实 `vip` 变更均完成为 `Applied`；部署
任务没有创建、重放或改写命令，也没有修改任何玩家身份。08:43 UTC 快照变为
`default=100 / vip=14 / admin=0 / owner=3`，09:03 UTC 跨过四个五分钟间隔后仍保持该
分布。最终 117 条快照、已绑定身份和用户等级映射的差异均为 `0`，活动命令为 `0`，同步
任务最近结果为 `0`。正常真实变更不回退门槛已经完成；隔离测试账号的四级正向、改回、
自身保护、最后管理员保护与拒绝路径仍作为外部验收保留。

回滚时停服后把当前 JAR 移出 `plugins`，优先从
`E:\manual-backups\luckperms-tier-agent-20260814T080510Z` 恢复 `0.1.2` JAR 与配置。
迁移 13 是加法变更，

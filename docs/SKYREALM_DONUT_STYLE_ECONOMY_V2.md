...
| `/shop` | 服务器出售基础物资并销毁金币 | 高于回收价的价格锚 | 本地候选已实现；未部署，价格不自动导入 |
| `/ah` | 玩家自由挂单与购买 | 玩家市场成交价 | `0.2.0` 候选已实现，未部署 |
| 手续费 | 抵消持续发币造成的通胀 | 上架费与成交税 | `0.2.0` 候选已实现，未部署 |
...
| 最低总价 | `1.00` 金币 |
| 普通玩家挂单数 | 最多 `5` 个活动挂单 |
| 活动成员挂单数 | 目标最多 `10` 个；`0.2.0` 候选暂与普通玩家一致为 `5` 个 |
| 协作者挂单数 | 目标最多 `15` 个；`0.2.0` 候选暂与普通玩家一致为 `5` 个 |
| 到期或取消 | 进入领取邮箱，不直接掉落到世界 |
| 购买一致性 | 金币扣款、卖家入账、税费、挂单成交必须在一个数据库事务中完成 |
| 物品安全 | 最终目标保存完整 Data Components/NBT；`0.2.0` 候选只接受无命名、附魔、容器和复杂组件的普通物品，按 `itemId + quantity` 托管 |
...
| --- | --- | --- |
| 余额 | `/money` | 我的金币 |
| 回收目录 | 当前 `/shop`；目标 `/prices` | 服务器回收，支持全目录模糊搜索 |
| 放入式回收 | `/sell` | 放入一组普通物品并确认回收 |
| 服务器商店 | `/shop` | 服务器商店 |
| 拍卖市场 | `/ah` | 玩家市场 |
| 我的挂单 | `/ah mine` | 我的挂单 |
| 领取邮箱 | `/ah claim` | 待领取 |
...
历史版本的 `/shop` 曾表示回收目录；当前候选已将回收入口迁移为 `/prices`，并让 `/shop`
独立承担服务器购买。候选尚未部署，不能把这段本地实现当作生产实时状态。

## 实施顺序

1. 保持生产商品目录为空，实现跨商品金额门禁、北京时间额度日和部分数量回收。
2. 在隔离数据库验证 `/prices` 与真正的 `/shop`，补齐首批价格、购买交付、失败补偿和货币销毁统计。
...
## 2026-08-18 玩家市场 0.2.0 候选

本地候选已形成 API `0.35.0`、HechaoEconomy `0.2.0` 和客户端 Screen `0.2.0`：

- `/ah` 支持玩家市场、放入式上架、我的挂单、购买确认、下架确认和待领取；
- 上架、购买、下架和领取均使用幂等操作，购买在一个数据库事务内完成买家扣款、卖家
  入账、`3%` 成交税、挂单状态和待领取写入；
- 上架费为 `1%` 且最低 `1.00` 金币，普通玩家最多 `5` 个活动挂单，挂单有效期 `24h`；
- 市场按 `serverId` 隔离，购买、下架和到期物品统一进入待领取，不直接掉落；
- 玩家市场、我的挂单和待领取均有模糊搜索；市场可按中文显示名、物品 ID、命名空间和
  卖家搜索，待领取可按中文显示名、物品 ID 和命名空间搜索；
- 现有 `/shop` 回收目录也改为服务端整份目录过滤，搜索不再局限当前 `45` 项批次；
- 客户端停止输入约 `0.4` 秒后提交搜索，减少逐字符命令和容器重绘。

该候选没有部署到 API、Minecraft 或客户端档案。生产仍为 API `0.34.0`、HechaoEconomy
`0.1.7` 和 Test Screen `0.1.10`。生产前必须在隔离 PostgreSQL 中运行迁移 `034` 和完整
挂单事务测试，并完成真人双账号的上架、搜索、购买、下架、到期和领取验收。复杂 NBT、
Data Components、模组能力、容器物品以及按等级增加挂单上限不属于本候选，不能在上线说明
中宣称已支持。

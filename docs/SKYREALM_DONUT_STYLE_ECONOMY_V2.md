...
个人和全服日限沿用 v1，单位均为物品数量。`/shop` 价格只表示服务器直接出售同一种物品的
价格；标记为“仅 AH”的稀有物品由玩家市场定价，服务器不出售。
...
1. 保持生产商品目录为空，先实现 `/prices` 与真正的 `/shop`，补齐购买交付和失败补偿。
2. 增加 Auction House 托管、事务、邮箱、上架费和成交税；完成复制物品与重放专项测试。
3. 将第三方屏幕改为服务器回收、服务器商店、玩家市场、我的挂单、待领取和市场数据。
4. 使用隔离数据库与假玩家完成发币、消费、挂单、购买、取消、到期、重复请求和恢复测试。
5. 只向 Test 通道发布；先开放 `/sell + /shop`，再按 `2/3/5/20` 人开放 `/ah`。
6. 以 7 天成交中位价、成交量、货币净增量和税费销毁量调价，不追随单个挂单。

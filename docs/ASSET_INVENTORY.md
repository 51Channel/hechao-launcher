...
- 启动器当前正式版本为 `0.15.14`，制品源码提交为
  ...
  两者均为 `NotSigned`。完整解决方案 `838` 项通过、`1` 项环境测试跳过，启动器
  `246/246`。
...
- `0.15.14` 私有 OSS 对象匿名读取 `403`、两轮签名回读 `200`，长度与 SHA-256 一致；
  ...
  `0.15.13` 保留为立即更新元数据回滚，其不可变对象与标签不得覆盖。
- 生产更新通道当前为 `LatestVersion=0.15.14`、
  `MinimumSupportedVersion=0.12.3`；API `0.37.0` 为 `active`、`NRestarts=0`，内外网健康、
  就绪和数据库均正常。认证会话确认 `0.15.13` 生成更新计划、`0.15.14` 不重复更新，
  完整下载长度和 SHA-256 一致。详细记录见
  [`LAUNCHER_RELEASE_0.15.14.md`](LAUNCHER_RELEASE_0.15.14.md)。

...
- 启动器当前正式版本为 `0.15.13`，制品源码提交为
  `8d019f9b409ab7f4b774900796ef8e877327af37`，正式标签为
  `launcher-v0.15.13`。单文件 EXE 为 `69,083,937` 字节，SHA-256 为
  `8368708E209955928821289A472FA5A35FA8FF475435E6F0CBA27D83F9CD7EB3`；
  NSIS 为 `61,999,226` 字节，SHA-256 为
  `48F7D8FFE06F4D6A7E5CB763FA63144A4A48E1DEE29857EC86931E19BA1CA257`。
  两者均为 `NotSigned`。完整解决方案 `837` 项通过、`1` 项环境测试跳过，启动器
  `245/245`。
- `0.15.12` 完成 DPAPI 持久登录和临时网络故障保留会话；`0.15.13` 使用最终赫朝品牌
  图标统一 WPF、EXE、安装器、卸载器和快捷方式。新 PNG SHA-256 为
  `83140B8FE8ECF6F7E560113D678378A9E079B47FDD007E903E3AFA2C3764B341`，七档 ICO
  SHA-256 为 `F15760FC927673E7A8980903AC44C233C22EE0CF27E8D252C2AECE4864D48E49`。
- `0.15.13` 私有 OSS 对象匿名读取 `403`、两轮签名回读 `200`，长度与 SHA-256 一致；
  `0.15.12 -> 0.15.13` 覆盖安装、全新安装、双轮卸载、设置和会话文件保留已验收。
  `0.15.12` 保留为立即更新元数据回滚，其不可变对象与标签不得覆盖。
- 生产更新通道当前为 `LatestVersion=0.15.13`、
  `MinimumSupportedVersion=0.12.3`；API `0.37.0` 为 `active`、`NRestarts=0`，内外网健康、
  就绪和数据库均正常。认证会话确认 `0.15.12` 生成更新计划、`0.15.13` 不重复更新，
  完整下载长度和 SHA-256 一致。详细记录见
  [`LAUNCHER_RELEASE_0.15.13.md`](LAUNCHER_RELEASE_0.15.13.md)。

...
| `127.0.0.1:8090` | `hechao-launcher-api.service` | 启动器 API `0.33.1`、赫朝账号、账号安全、基础设施角色、论坛会话联动、受控全局等级与代理协议门禁、生产档案、诊断上传、客户端兼容保护、运行遥测、服务器深度指标、统一告警、精确服控交接、活动企划、动态部署槽与经济商品目录 |
...
- 启动器当前正式版本为 `0.15.9`，制品源码提交为
  `906fdb848eb3805e05b95d01b569f32379fd3cd4`，正式标签为
  `launcher-v0.15.9`。单文件 EXE 为 `69,053,206` 字节，SHA-256 为
  `8B703CD1A02DEF6938586516FA69A2A1493C5B75B57F3E0818F4972BD81CAD1F`；
  NSIS 为 `61,995,538` 字节，SHA-256 为
  `88AE1869A47E313291AF0C60F8984BF14C1B5D0719291D72DDA254870487DE79`。
  两者均为 `NotSigned`。完整解决方案 `806` 项通过、`1` 项环境测试跳过，启动器
  `234/234`。
- `0.15.9` 私有 OSS 对象匿名读取 `403`、两轮签名回读 `200`，长度与
  SHA-256 一致；`0.15.8 -> 0.15.9` 覆盖安装、全新安装、双轮卸载、设置和
  会话文件保留已验收。`0.15.8` 保留为上一正式版本，其不可变对象与标签不得覆盖。
- 生产更新通道当前为 `LatestVersion=0.15.9`、
  `MinimumSupportedVersion=0.12.3`；API `0.33.1` 为 `active`、`NRestarts=0`，
  内外网健康与就绪端点均为 `200`。认证会话确认 `0.15.8` 生成更新计划、`0.15.9`
...
- API `0.30.5-20260814T121420Z` 单文件 SHA-256 为
  `7532B2EFC276455486415181FC7E361527D3622D9ADF667BF89FD450E21AE4B8`，归档
  SHA-256 为 `CA39C1C6E7D7CC124581E72DF2E3FE523D70F3E1C5226065A17F1B1361C6B496`。
  服务器目录和整合包部署结果可精确交接到单服控制页，入口不会自动提交启动命令；
  直接程序回滚目标为 `0.30.4-20260814T093000Z`，详细记录见
  [`API_RELEASE_0.30.5.md`](API_RELEASE_0.30.5.md)。
- API `0.30.6-20260814T133415Z` 单文件 SHA-256 为
  `6CF985C4EEC6299A393C344A26BE031D6CB641C4790B40E54EE9A2EE870D353E`，归档
  SHA-256 为 `17B6995E8349A6024BDEA26AD84B86F578FC3EBB918ED7B3D4DA6F0F914D5025`。
  活动企划后台会只读显示旧目录排期和缺失项，不会自动发布、部署或启停 Minecraft；
  直接程序回滚目标为 `0.30.5-20260814T121420Z`，详细记录见
  [`API_RELEASE_0.30.6.md`](API_RELEASE_0.30.6.md)。
- API `0.30.7-20260814T144949Z` 单文件 SHA-256 为
  `6523572993E4848B2BD1CA0E743FBE10E4A07FE0F617BDCD3B514C39225F2190`，归档
  SHA-256 为 `21894901792A8791BA89F704ED74A1144360CBD70974F573673246F5591D36AC`。
...
- API `0.33.1`、基础设施角色、客户端兼容保护、日志脱敏、Publisher 工作空间保护、成员问卷正版资格桥接、活动企划、LuckPerms 代理协议门禁、活动预下载与进服权限分离、后台精确服控交接、旧活动排期只读显示、动态部署槽及经济商品目录已部署；启动器 `0.15.9` 为私有 OSS 当前版本。管理员 Web 已启用，真实 MFA 已登记。
...
- 数据库迁移：启动时迁移 `1` 至 `31` 校验全部通过；`0.31.0` 新增动态部署槽迁移，`0.32.0` 新增独立槽迁移，`0.33.0` 新增经济账本与商品目录迁移

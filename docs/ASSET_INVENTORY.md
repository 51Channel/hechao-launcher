...
| `127.0.0.1:8090` | `hechao-launcher-api.service` | 启动器 API `0.6.0`、真实目录、认证会话、签名分发、Velocity 授权与状态心跳 |
...
- 发布 ID：`0.6.0-20260723T123346Z`
- 本地 API 源码为 `0.9.0`、启动器源码为 `0.8.0`，尚未部署；线上 API 仍为 `0.6.0`，签名清单、客户端分发、Velocity 授权和状态心跳均保持原部署状态
...
- 数据库迁移：`initial_catalog_and_identity`、`authentication_and_luckperms`、`velocity_authorization`、`server_heartbeats`

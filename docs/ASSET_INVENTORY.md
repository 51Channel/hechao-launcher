...
- 启动器 `0.9.1` 基于提交 `6adc93a` 增加 Minecraft 正常/异常退出记录、异常提示和玩家主动生成的本地脱敏诊断包；日志截尾、固定 ZIP 条目、世界存档排除、重解析点拒绝与 20 条退出/10 个诊断包保留上限均有自动测试。签名管线启用前的单文件 EXE 候选为 `68,591,719` 字节，SHA-256 为 `98C53E741F4880258568AA1EBC042E30102467AD139CF6A0AA8CCD02D484121F`；NSIS 安装包为 `61,795,645` 字节，SHA-256 为 `99E42CC78157050041F13504AE9ED93F1F1573D7244F06EA5E692EBF8291929F`。Windows 安装/卸载冒烟测试和 `140/140` 解决方案测试已通过；这两份历史候选均为 `NotSigned`，尚未上传或向玩家分发，不能作为正式发布物。
...
6. Windows 正式构建已加入启动器、NSIS 卸载程序和外层安装包的 Authenticode 签名、时间戳、验签与签名后哈希关卡；当前构建机已安装并验证 Microsoft Windows SDK SignTool，但尚无公网受信任代码签名证书，因此只能生成明确标记的未签名开发包，不能向玩家发布。客户端清单的 ECDSA 签名不能替代 EXE 代码签名。

## 5. 当前 API 部署状态

- 发布 ID：`0.9.0-20260723T195253Z`
- API `0.9.0` 已部署；启动器 `0.9.1` 为未分发安装包候选。管理员 Web 代码已进入生产二进制但功能保持关闭

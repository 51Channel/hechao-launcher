...
- API `0.20.0` 与平台监控器 `0.1.0` 已部署；启动器 `0.11.14` 为私有 OSS 灰度版本。管理员 Web 已启用，真实 MFA 已登记：凭据 `1`、恢复码哈希 `8`、有效 MFA 会话 `1`
...
- 诊断上传：编号 `1e707520` 已由真实 `0.11.14` 主动确认上传；本地和生产端均为 `707` 字节且 SHA-256 为 `1C53C309DDA3D1D9A905836E79A041EDCD4DDD03C543E0424119C876AAA6BF92`，上传授权/完成审计存在，管理员下载审计待完成

...
- 启动器当前正式版本为 `0.13.7`，制品源码提交为
  `b6fa703d03359519ae5d73336dc974409cf80937`，正式标签为
  `launcher-v0.13.7`。单文件 EXE 为 `68,856,376` 字节，SHA-256 为
  `6C98FF66450E9FF8F91A00945BB7F7F8BF12200E071E82D34F978B751C21860A`；
  NSIS 为 `61,910,615` 字节，SHA-256 为
  `5E7DD73BED96BE98EEB616CCF8155D268B92C8A0A25B54BB4D67F0712429B5CF`。
  两者均为 `NotSigned`。完整解决方案 `458/458`，启动器 `135/135`。
- `0.13.7` 私有 OSS 对象匿名读取 `403`、两轮签名回读 `200`，长度与
  SHA-256 一致；`0.13.6 -> 0.13.7` 覆盖安装、全新安装、双轮卸载和状态
  保留已验收，正式自更新点击待当前游戏会话结束后执行。`0.13.6` 保留为上一
  正式版本，其不可变对象与标签不得覆盖。
- 生产更新通道当前为 `LatestVersion=0.13.7`、
...
- 服控代理 `0.2.1` ZIP 为 `33,140,634` 字节，SHA-256
  `DBF3B48695C60139A5631A52491CA017A7B0382807D03E5DAFE838A7A7239DF0`；
  EXE 为 `73,899,214` 字节，SHA-256
  `2D7D334C2205EB5F5D4032586B040F3624A85FA4B711630F151E5C8067D5C700`。
- owl5 和 owl9 均运行代理 `0.2.1`；9 个目标已上报 JVM 内存，五个运行目标 PID
  在部署前后不变。回滚备份、心跳隔离和每服内存基线见
  [`SERVER_CONTROL_AGENT_RELEASE_0.2.1.md`](SERVER_CONTROL_AGENT_RELEASE_0.2.1.md)。

...
| `127.0.0.1:8090` | `hechao-launcher-api.service` | 启动器 API `0.30.5`、赫朝账号、账号安全、基础设施角色、论坛会话联动、受控全局等级与旧代理协议门禁、六份生产档案、诊断上传、客户端兼容保护、运行遥测、服务器深度指标、统一告警与精确服控交接 |
...
- 生产更新通道当前为 `LatestVersion=0.15.7`、
  `MinimumSupportedVersion=0.12.3`；API `0.30.5` 为 `active`、`NRestarts=0`，
...
- 发布 ID：`0.30.5-20260814T121420Z`
- API `0.30.5`、基础设施角色、客户端兼容保护、日志脱敏、Publisher 工作空间保护、成员问卷正版资格桥接、活动企划、LuckPerms 代理协议门禁与后台精确服控交接已部署；启动器 `0.15.7` 为私有 OSS 当前版本。管理员 Web 已启用，真实 MFA 已登记。
...
- 数据库迁移：启动时迁移 `1` 至 `28` 校验全部通过；`0.30.5` 没有新增迁移

...
> 源码版本：API `0.10.1`（本功能自 `0.7.0` 起，`0.10.1` 保持兼容）
> 生产状态：API `0.10.1` 与迁移 5 已部署；管理员 Web 保持关闭，因此目录管理端点尚未对外启用
...
本功能代码与数据库结构已部署，但公网管理入口未启用。正式激活前必须：

1. 生成并校验目标 API Linux 发布物、提交号与 SHA-256；当前生产基线为 `0.10.1-20260724T102830Z`。

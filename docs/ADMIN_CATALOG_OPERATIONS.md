...
`GET /v1/public/activities` 是独立的只读公开投影，不属于管理员端点。它只返回可见的
活动目录记录及官网展示需要的名称、公告、排期、解析后状态、人数上限、Minecraft
版本、加载器和最低等级；不会返回客户端档案 ID、清单摘要、下载地址、账号例外、服控
目标或审计数据。赫朝官网通过同机 `127.0.0.1:8090` 读取该投影，并以 30 秒短缓存合并
到公开日程与 ICS。目录后台仍是活动服排期的唯一写入口，官网不反向修改目录。
...
可见且归类为活动的目录记录会同时进入官网公开活动投影。没有 `opensAt` 和 `closesAt`
的记录仍可由启动器展示，但不会占用官网日历日期；设置任一排期边界后，官网会在下一次
短缓存刷新时出现。官网读取失败时保留自身活动与企划日程，并优先沿用进程内上次成功的
启动器活动快照，不把目录故障扩大为整站故障。

最终访问优先级为：禁用账号、未绑定 Minecraft、已归档服务器、非在线状态、
有效拒绝规则、有效允许规则、LuckPerms 数据新鲜度、全局最低等级。到期规则自动
退回等级判定；允许规则不能绕过账号禁用、未绑定、归档或服务器关闭。

从 API `0.30.7` 起，活动目录可见性、客户端下载和进服权限是三个独立结果：

- 所有已登录账号都能看到 `velocity_target=activity` 的可见玩家活动，并可提前下载其
  当前可解析的签名客户端档案；隐藏活动、基础设施记录和没有可用发布的档案仍不开放；
- 目录响应使用 `canJoin` 表示当前账号是否满足最低称号或单服例外权限。有效 `Deny`
  优先拒绝，有效 `Allow` 优先允许，否则比较账号等级和 `minimumTier`；
- 等级不足的活动仍进入活动日历和客户端下载流程，但启动器在客户端就绪后禁用进服；
  永久服务器继续按原规则隐藏整条无权目录记录；
- `canJoin` 只是启动器提示和本地门禁，不能替代 Velocity 对账号状态、正版身份、服务
  状态、单服规则、权限新鲜度、一次性授权和客户端兼容性的最终复核。

### 2.1 新增服务器自动发现

`0.24.2` 起，“新增服务器”抽屉会复用服控 overview 与运行监控数据，列出同时满足
以下条件的目标：

- `agentConnected=true`，即最后心跳仍在服控新鲜度窗口内；
- `online=true`，即服务端进程和端口当前真实在线；
- `serverId` 尚不存在于完整服务器目录，包含已归档记录。

管理员选择目标后，页面自动填写服务器 ID、显示名/短名初值、图标初值、在线状态、
最大人数、Velocity 目标和监控开关；能够从运行监控识别时再填写 Minecraft 版本与
加载器。客户端档案、展示名称、加载器和 Velocity 目标必须在保存前人工核对。

该功能只读服控状态，不创建代理目标、不启动或停止 Minecraft，也不会自动提交表单。
owl9 必须继续遵守 `pvp` 为恐怖整蛊、`pvp-purpur` 为真正 PVP 的命名边界；不得根据
显示名合并或替换这两个 ID。
...
8. 回归 `hechao.world/schedule`、官网 ICS、`api.hechao.world`、启动器目录、分发和心跳；API 必须先于依赖该投影的官网版本切换。

...
2026-08-11 的 API `0.30.2` 已生产部署上述只读资格端点，并继续复用论坛桥接令牌、
回环来源限制和 `internal-forum` 限流。API 测试 `305/305`、完整解决方案 `725/725`
通过；生产 29 个网站映射账号均返回 `200`，其中 13 个满足完整正版资格，不存在账号
返回 `404`，缺少桥接凭据返回 `401`。验证记录只保存聚合计数，不保存账号身份。

正式 release 为 `0.30.2-20260811T124943Z`，标签为 `api-v0.30.2`。部署顺序仍必须是
API 先于网站；API 回滚到不含该端点的版本会让新成员问卷返回受控 `503`，不会绕过正版
门禁。完整记录见 [`API_RELEASE_0.30.2.md`](API_RELEASE_0.30.2.md)。

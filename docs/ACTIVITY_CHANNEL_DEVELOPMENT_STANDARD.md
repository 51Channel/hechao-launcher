...
> 开发者和 Codex。
>
> 核心决策：不同活动可以使用不同 Minecraft 版本、加载器、客户端档案和真实承载槽，
> 玩家统一从启动器经 Velocity 进入；启动器是唯一换服入口。
...
共享旧替换入口的活动使用 Velocity 目标 activity；独立槽使用与 serverId 相同的目标。
不得借用 survival2、lobby 或历史 pvp 作为活动别名。
...
活动企划是玩家看到的逻辑记录，真实承载槽是服控、进程、端口和部署记录。企划必须通过
`activityTargetServerId` 绑定一个真实槽：共享旧替换入口时选择固定 `activity`；需要与
其他服务器并行时选择独立槽，并保持槽自身 `velocityTarget=<serverId>`。逻辑企划不得
作为普通服务器目录项或服控目标。

同一真实承载槽上的已发布企划按 `[开始, 结束)` 互斥，不同独立槽可以同时发布。企划
发布后，玩家目录只显示企划卡并隐藏该真实槽的重复卡片；运行状态、在线人数、部署身份、
Velocity 目标和动态回环端口均从真实槽解析。玩家直接请求真实槽 ID 时也必须经过同一
企划占用门禁，不能绕过排期、客户端或权限。

推荐切换顺序：

1. 建立或核对真实承载槽，确认目录、服控目标、端口、Velocity 目标和冲突组一致。
2. 确认客户端档案已经完成要求的 Test/Gray/Production 阶段，并绑定正确 `profileId`。
3. 在活动企划中选择该真实槽；同槽已有已发布排期时先调整时间、撤回或归档旧企划。
4. 等待目标玩家清空，保存并备份世界和配置；通过服控代理停止真实槽，失败则终止部署。
5. 从企划页部署精确整合包，等待槽心跳回报相同 import、档案和版本；部署后保持停止。
6. 发布企划，重新读取普通目录、活动企划和玩家目录，确认没有逻辑记录与真实槽重复。
7. 只有获得明确授权才启动真实槽，并取得新鲜心跳、指标和单一端口监听。
8. 用管理员真实账号申请 fresh grant、启动正确档案并进入企划；直接请求真实槽 ID 必须
   不能绕过企划门禁。
...
玩家入口：固定替换槽填 velocityTarget=activity；独立槽填 velocityTarget=<serverId>

...
- 对照版本：Screen `0.2.7`、HechaoEconomy `0.2.2`、API `0.36.0` 本地候选
- 生产边界：本次不上传 OSS、不导入后台、不切换客户端通道、不启停任何服务
...
- 当前 `0.2.2/0.2.7` 仍是本地候选，没有上传 OSS、导入后台或部署；
...
- 未实现：生产部署、OSS 上传、Test 通道切换、订单簿、成交历史、碎片账本、真正 `/shop`。
- 回滚：代码回滚到本轮前提交即可；候选制品和既有 Test `1.0.25 / Screen 0.2.5` 不被覆盖。

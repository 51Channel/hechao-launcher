...
- 对照版本：Screen `0.2.8`、HechaoEconomy `0.2.3`、API `0.36.1`
- 发布状态：服务端 Screen `0.2.8` 已冷部署，客户端档案 `1.0.27` 已仅切 Test `100% / r20`
...
- `0.2.8 / 1.0.27` 已完成协议 `3` 成对冷部署并进入 Test `100% / r20`；
...
1. 同步发布（已完成）：服务端 Screen `0.2.8` 已冷更新并通过协议 `3`、唯一 JAR、命令和日志验收，Test 已切到档案 `1.0.27 / r20`。
...
- 已实现：14 项快捷首页、服务端筛选、会话级授权/拒绝、转账与 TPA 表单、精确回执、
  等待/未知状态锁定和紧凑窗口状态区；
- 已发布：API `0.36.1`、HechaoEconomy `0.2.3`，服务端 Screen `0.2.8`，以及仅 Test 的
  客户端档案 `1.0.27 / r20`；Gray 与 Production 未分配；
- 已完成：服务端零玩家保存、正常停服、完整离线备份、协议 `3` 冷启动验收、档案导入和
  Test 切换；
- 未完成：启动器真实增量下载、14 项菜单目视、双账号转账/TPA/市场、断线、背包竞争、
  幂等和余额守恒门禁；订单簿、成交历史、碎片账本和真正 `/shop` 也不在本轮；
- 回滚：客户端 Test 恢复 `1.0.26 / Screen 0.2.7`，服务端从
  `E:\manual-backups\activity-survival-screen-0.2.8-20260822T114459Z` 恢复；协议两端必须
  成对回滚，不删除不可变清单和对象。

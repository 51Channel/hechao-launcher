...
> 状态：生产基线为 API `0.32.1` 与 owl5/owl9 代理 `0.7.2`。
...
### 6.14 全权限游戏控制台（双机代理 0.7.2）

`0.7.2` 允许配置 `allowedCommandPrefixes=["*"]`，放行全部 Minecraft、模组和插件
命令，包括 `op/deop`、LuckPerms 和命名空间命令。`stop`、`restart`、`shutdown`、
`end` 仍在 API、后台和本机代理三层拒绝，必须走结构化停止或重启。

API `0.32.1` 先上线并验证旧代理心跳后，owl5 七个固定目标、一个动态槽及 owl9 两个
固定目标全部切换为 `*`。owl9 首次尝试的 `0.7.1` 暴露了禁用动态槽时仍规范化空根

...
> 源码版本：启动器 `0.10.0`、API `0.10.1`
> 生产状态：API `0.10.1` 与迁移 6 已部署；`AdminWeb` 未配置且默认关闭，入口仍返回 404
...
1. 生成目标 API 与启动器安装包并核对版本、提交号和 SHA-256；当前 API 基线为 `0.10.1`，启动器候选为 `0.10.0`。
...
API `0.10.1` 已生产部署，但本次账号安全发布没有启用管理后台，也没有重启任何 Minecraft 或 Velocity 进程。

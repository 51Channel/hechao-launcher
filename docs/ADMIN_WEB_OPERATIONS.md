...
1. 生成目标 API 与启动器安装包并核对版本、提交号和 SHA-256；当前 API 基线为 `0.12.0`，启动器候选为 `0.11.6`。
...
应用故障时可把 API `current` 链接切回已验证的 `0.11.3-20260725T195000Z`。迁移 5 和迁移 6 都是加法变更，旧 API 不读取新增表与字段，回滚时不要删除 MFA、会话或审计记录。

...
> 前端状态：Vue 3、TypeScript、Vite 与 Vue Router；第十二页“经济监控”含总体行情和单品 K 线，已完成本地自动与视觉验收，尚未部署生产
...
本地候选已通过完整 .NET 解决方案测试 `815/815`，其中 API `368/368`（另有隔离
PostgreSQL 集成测试因本机未配置测试库而明确跳过）、Vitest `19/19`、Playwright
`34/34`、十二路由 WCAG A/AA、1440px 与 390px K 线截图和横向溢出检查。正式上线仍须
先备份 PostgreSQL，应用迁移 032、033，以两次不同价格的真实回收验证开高低收，再完成
健康、日志、静态资源和直接回滚验收；当前文档不把本地候选写成已经生产部署。

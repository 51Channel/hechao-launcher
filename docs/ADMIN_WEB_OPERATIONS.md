...
> 当前生产：API `0.34.0`，`AdminWeb__Enabled=true`
> 生产状态：真实管理员已完成 MFA 和可信设备验收，十二页 Vue 后台、活动企划月历与经济监控均已部署
> 管理入口：`https://admin.hechao.world/admin/`
> 前端状态：Vue 3、TypeScript、Vite 与 Vue Router；第十二页“经济监控”含总体行情和单品 K 线，已完成自动、视觉与生产静态资源验收
...
`GET /v1/admin/economy/overview?hours=<24|168|720|2160>&serverId=<可选>` 展示总体经济行情，并通过
`GET /v1/admin/economy/items/history?itemId=<物品ID>&hours=<24|168|720|2160>&serverId=<可选>`
读取单个物品行情。
页面使用 ECharts 绘制全局货币总量折线和区间新增货币柱线，并保留摘要、财富分布、
玩家余额、物资回收和服务器流量表，因此图表不可用时仍有可读数据。

单品区可搜索商品目录中的原版或模组物品 ID，也可从物资回收排行直接进入。每个小时或
每天的 K 线由已提交报价按成交操作时间计算真实开盘、收盘、最低和最高价格；上涨为红色，
下跌为绿色。当前目录价、窗口首末价、涨跌、最低/最高、成交数量、金额、卖家和笔数继续
以文本指标展示。没有成交的时间桶保持空缺，不沿用旧价，也不生成演示蜡烛。

当前经济系统只有官方回收，没有玩家拍卖行或自由市场，因此该区域明确命名为“单品官方
回收行情”。服务器筛选只影响已完成成交；当前目录价格仍是全局商品配置，不能解读为
服务器独立市场价。
...
账本净额为零，因此不会错误增加货币供给。迁移 032 增加总体行情索引，迁移 033 增加
`item_id + committed_operation_id` 的单品历史索引；两者都不修改迁移 031 的交易写入合同。

API `0.34.0` 已完成生产部署。完整 .NET 解决方案 `815/815`、API `368/368`（另有隔离
PostgreSQL 条件测试因本机未配置测试库而跳过）、Vitest `19/19`、Playwright `34/34`、
十二路由 WCAG A/AA、1440px 与 390px K 线截图和横向溢出检查均通过。生产迁移为
`33/33`，四个分析索引和 `85/85` 商品均已回读；页面、K 线静态资源、本机与公网健康、
日志和直接回滚均通过。生产当前有 `0` 个经济账户、操作和报价，因此真实成交 K 线仍为
空状态；待首批真实回收后再补有数据的生产目视验收，不用测试成交污染正式账本。
...
5. 部署当前候选 API 后确认迁移 5、迁移 6、迁移 10、迁移 11、迁移 15、迁移 16、迁移 19、迁移 20、迁移 21、迁移 28、迁移 31、迁移 32、迁移 33、`healthz` 和 `readyz`。

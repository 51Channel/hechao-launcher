...
1. 生成目标 API 与启动器安装包并核对版本、提交号和 SHA-256；当前 API 基线为 `0.16.0-20260726T222124Z`，启动器源码候选为 `0.11.12`。
...
API `0.16.0` 已生产部署，管理后台开关已启用，但真实管理员尚未完成首次 TOTP 与恢复码保存。在登记完成前不得执行管理写入，也不得把开关开启视为 MFA 已验收。

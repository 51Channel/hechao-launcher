...
1. 生成目标 API 与启动器安装包并核对版本、提交号和 SHA-256；当前 API 基线为 `0.14.1`，启动器源码候选为 `0.11.12`。
...
应用故障时可把 API `current` 链接切回已验证的 `0.13.0-20260726T173536Z`。迁移 5、迁移 6 和迁移 10 都是加法变更，旧 API 不读取新增表与字段，回滚时不要删除 MFA、会话、访问规则或审计记录。

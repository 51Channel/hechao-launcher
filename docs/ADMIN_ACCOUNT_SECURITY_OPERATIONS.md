...
> 目标版本：API `0.16.0`
>
> 当前状态：真实管理员 MFA 已于 `2026-07-27` 完成并写入生产审计；论坛撤销生产链路通过，仍待账号安全页面逐项操作与大厅代理加载验收
>
...
生产一次性账号进一步验证论坛直接请求幂等、API worker 投递和精确清理。大厅代理 JAR
和配置已部署，但没有重启大厅。完整发布 ID、制品哈希、备份和证据见
[`API_RELEASE_0.16.0.md`](API_RELEASE_0.16.0.md)。当前生产 MFA 凭据数仍为 `0`，
因此第 8 步和第 9 步的游戏内等级改回仍待执行。

## 6. 回滚

迁移 `11` 至 `13` 都是加法变更。应用可以切回 `0.15.0-20260726T202540Z`，
旧版本会继续执行 UUID 封禁，但会忽略论坛撤销 outbox 和等级命令。因此回滚前必须核对
没有未完成投递或等级命令。故障回滚不删除封禁、outbox、命令或审计记录。

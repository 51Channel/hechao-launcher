# 天域远征工业季玩家市场 0.2.2 候选

- 候选日期：`2026-08-20`
- 状态：候选已完成发布；API 0.36.0 与插件 0.2.2 已部署，客户端档案 1.0.26 已导入并仅切换到 Test `100% / r19`
- 后续正式记录：[SKYREALM_ECONOMY_SCREEN_RELEASE_0.2.7.md](SKYREALM_ECONOMY_SCREEN_RELEASE_0.2.7.md)、
  [SKYREALM_INDUSTRIAL_PROFILE_RELEASE_1.0.26.md](SKYREALM_INDUSTRIAL_PROFILE_RELEASE_1.0.26.md)
- API 源码版本：`0.36.0`
- 服务端插件：`HechaoEconomy-0.2.2.jar`
- 客户端模组：`HechaoEconomyScreen-NeoForge-1.21.1-0.2.7.jar`
- 数据库迁移：无新增迁移，兼容既有 `034_economy_player_market`

## 交易状态声明

本候选版本已经明确实现玩家交易合同，但尚未完成线上真人验收，两个结论必须分开：

| 层级 | 当前结论 | 依据 |
| --- | --- | --- |
| 代码合同 | 已具备交易能力 | `/ah` 浏览、上架、购买、下架、待领取和 `/pay` 均有服务端路由；服务端检查 API、Vault、命令所有权并失败关闭 |
| 自动化验证 | 已通过 | API 完整套件 `375` 通过；隔离 PostgreSQL 交易验收 `1/1` 通过；Bukkit `30/30`；Screen `91/91` |
| 当前运行线上 | 已进入 Test，真人交易待验收 | API、插件和客户端档案已完成发布；双账号真人交易门禁尚未完成 |
| 玩家可用结论 | 待 Test 双账号验收 | 必须实际完成上架、搜索/排序、购买、下架、待领取、断线、背包竞争、幂等重试和余额守恒 |

因此，本文件中的“可交易”表示**实现和自动化合同成立**，不表示玩家已经能在当前线上服完成交易。
未完成真人门禁前，不得把本候选推进 Gray 或 Production。

## 本轮功能

本轮同时收口了天域远征快捷菜单的流程边界：

- 市场上架统一消费服务端一次性菜单会话，不再由客户端绕过会话直接发起首页动作；
- 会话只在动作合法、未过期且通过限速后消费，非法动作和过快点击不会误删合法会话；
- 暂停菜单按实际 Mods 按钮宽度拆分，并防止 Screen 重建时重复插入入口；
- 快捷首页按“账户、服务器回收、玩家市场 / 市场上架、队伍、设置”分组，资产摘要显示可用与冻结余额；
- 业务等待页在菜单失效、限速或不可用时立即显示错误，并提供可恢复的重试/返回路径。

本轮按 DonutSMP 的 Auction House 交互研究，先把玩家市场的读取排序和单位价信息做成一个
完整切片。挂单写入、购买、成交税、下架、到期、待领取、幂等键和 `serverId` 隔离均未改变。

API `GET /v1/internal/economy/market/listings` 新增白名单参数 `sort`：

- `recently_listed`：最新上架，按 `created_at DESC, listing_id` 稳定排序；
- `lowest_unit_price`：单位价从低到高，按精确的 `total_price / quantity` 排序；
- `highest_unit_price`：单位价从高到低；
- `expiring_soon`：剩余时间从短到长。

省略 `sort` 或传空值仍使用 `recently_listed`。未知值返回验证错误，排序表达式只来自服务端
枚举，不拼接客户端原始输入。响应新增计算字段 `unitPrice`，以总价除以数量并四舍五入到
四位小数；原有字段和写入合同保持不变。

## 游戏内交互

- Bukkit 市场菜单底部空槽 `51` 变为排序控制，点击后循环四种排序并异步刷新；刷新期间按钮
  显示处理中状态，失败会恢复上一种排序，不重复发送请求；
- 挂单 Lore 同时显示总价和单位价，完整卖家、数量、剩余时间和操作提示仍保留；
- NeoForge Screen 在标题栏搜索框旁显示紧凑排序按钮，窄窗口会收缩搜索框而不是重叠；
- 市场卡片在空间足够时显示总价和单位价两行，原生 Tooltip 仍可查看完整 Lore；
- 旧客户端命令、旧 API 请求和省略排序参数的旧插件调用继续默认使用最新上架。

## 本地验证

- API 完整测试：通过 `375`，失败 `0`；另用仓库隔离 PostgreSQL 脚本执行交易验收 `1/1`，
  失败 `0`；临时数据库和角色清理后计数为 `0:0`；
- HechaoEconomy：PowerShell 7 + Java 21 执行 `clean test build --no-daemon`，`30/30`
  测试和构建通过；JAR `443,345` 字节，SHA-256
  `37A91D49FEBFC2B90723E0E12DE2E36AFEA5DF835241AD0272D107C6D34EFEEF`；
- Screen：PowerShell 7 + Java 21 执行 `clean test build --no-daemon`，`91/91`
  测试和构建通过；JAR `937,825` 字节，SHA-256
  `71E3FE9C74BBBF439AC91461E564E43096B7EA7E2D7DB3585CE45FFC8383A658`；
- `git diff --check`：通过。

## 发布后门禁

1. [x] 在隔离 PostgreSQL 运行迁移 `031-034` 和排序读取、单位价 JSON、市场事务全套测试；
2. [ ] 用两个测试账号确认最新、低价、高价、临期四种排序在搜索、分页和重复点击下稳定；
3. [ ] 在 `512 x 270`、`200 x 140` 和高 DPI 客户端中完成真实 Screen 目视检查，确认搜索框、
   排序按钮、物品卡和 Tooltip 不重叠；
4. [x] 按既有流程制作新的不可变客户端档案，只推进 Test，`1.0.25` 保留为回滚目标；
5. [ ] 完成双账号购买、下架、待领取、断线、背包竞争、幂等重试和余额守恒后，才评估 Gray。

本版本已进入 Test，但仍不把真人交易、成交历史、订单簿、真正 `/shop` 或 shards 统计
宣称为已正式验收或已实现。

...
1. Minecraft Java API 许可审核通过，并使用真实正版账号完成绑定、下载、启动和进服验收。
2. 管理员手动重启 Velocity，使已放置的授权插件先以 `monitor` 模式加载。
...
上传安装包、建立官方下载地址或向内部成员开放下载均属于生产分发动作，必须单独确认。

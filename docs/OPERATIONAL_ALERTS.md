...
> 当前状态：API `0.22.0`、状态采集器 `0.2.1`、平台监控器 `0.1.2` 已生产部署
>
> 更新日期：`2026-07-30`
>
> owl9 边界：告警目标 `pvp` 当前代表恐怖整蛊服 `C:\mc\server`；
> `E:\MinecraftServer` 的真正 PVP 服尚未接入告警。
...
- 开服前历史基线与当时真实状态一致：活动服和 PVP 离线为 Critical；大厅、
  Survival1 和 Survival2 尚未加载指标代理，因此 Tick 指标为 Warning。该快照仅保留为
  告警链路验收记录；2026-07-28 受控开服后五个目标均已加载指标并取得 TPS/MSPT/GC。

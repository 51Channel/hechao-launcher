...
> 状态：2026-08-16 发现的两个阻断已在 `0.1.3` 修复；候选已验证，尚未生产部署
> 审查日期：2026-08-14
...
- NeoForge 屏幕的“服主回收设置”现在执行命名空间动作
  `hechaoeconomy:heco product`，Bukkit 路由允许无子参数进入快捷设置。
- 数据库使用新增迁移 `031`，商品 ID 约束与 C# 规则一致，支持原版和普通模组命名空间。
- 屏幕协议 `2` 只传 `sessionId + actionIds`，标题、说明和按钮文案固定在客户端；
  管理员权限最终由 Bukkit/LuckPerms 裁决，不再错误依赖 OP 等级。

修复版为 HechaoEconomy/Screen `0.1.3` 和客户端档案 `1.0.12`。客户端与服务端的屏幕
协议不兼容旧 `0.1.2`，部署时必须协调升级，不能只更新一端。

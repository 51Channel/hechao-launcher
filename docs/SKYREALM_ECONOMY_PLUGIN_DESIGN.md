...
NBT、数据组件或模组能力的物品永久排除在官方回收之外，只能在后续玩家市场中通过完整
物品托管交易。对应的序列化与恢复仍属于后续 Bridge 范围。
...
带组件或其他元数据物品始终拒绝。它们以后进入玩家市场，但不会成为 `/sell` 商品。
...
- 标准物品收购订单；
- 冻结余额、离线交付和价格历史；
- 改名、附魔、耐久、容器及其他带 NBT/数据组件物品由玩家自由定价，市场只转移已有货币，
  不通过成交铸造新货币；
- 只允许精确兼容组跨服领取物品。
...
- 经过产量审计后，只逐项开放无自定义数据的模组基础材料回收；带 NBT、数据组件或模组
  能力的物品只开放玩家市场托管。

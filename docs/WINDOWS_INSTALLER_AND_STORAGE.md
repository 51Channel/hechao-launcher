...
| `shared/player-settings` | 跨档案共享的玩家 `options.txt` 设置主副本 | 默认保留 |
...
`0.14.0` 起，启动器把灵敏度、视野、音量、按键绑定等玩家设置合并到
`shared/player-settings/options.txt`。启动器打开时会导入各档案最后修改的
`options.txt`，进入服务器前写入目标档案，游戏退出后再次回收。即使玩家设置为
“游戏启动后关闭启动器”，下次打开启动器也会补做导入。

共享采用按键合并，而不是让多个 Minecraft 进程直接写同一个文件：

- `resourcePacks`、`incompatibleResourcePacks`、`serverResourcePacks`、`lastServer`
  和 options 数据版本不跨档案覆盖；
- 已存在的目标档案只更新它已有的按键项，避免把某个活动模组的专用按键写入其他档案；
- 尚无 `options.txt` 的新档案会用共享主副本初始化，使基础按键第一次进入时生效；
- 共享主副本采用同目录临时文件替换，读取或写入失败时取消启动或删除。

服务器详情中的“删除客户端”会在跨进程安装锁内删除当前客户端、档案专用 Java、
回滚副本和该档案遗留的 staging 目录。它不会清理 `shared/objects`、
`shared/player-settings`、账号会话或其他档案。游戏正在运行、安装/回滚进行中或
个人设置尚未安全导入时，删除会被拒绝。
...
安装、修复、删除和启动开始时必须冻结服务器、档案、数据根目录、Java 与内存配置。
操作完成前不得让目录切换、设置重置或切服改变这份上下文。目录加载和本地状态检查使用
generation、取消令牌与目录快照，晚返回的旧请求不得覆盖当前目录状态。

运行状态必须保存游戏实际使用的 `DataRoot`。切服前和游戏退出后，
`shared/player-settings/options.txt` 均从真实运行目录同步，不能依赖此时界面选择的数据目录。
...
自更新安装失败时保留当前可运行版本并记录失败版本。下次启动不得对同一失败版本自动形成
下载、重启、失败的循环；界面保留明确的手动重试入口。新的目标版本仍按正常启动检查处理。

...
> 当前生产 API：`0.16.0-20260726T222124Z`
> 当前启动器私有 OSS 灰度候选：`0.11.12`
> 当前 Velocity 插件：`0.2.0`（`monitor`）
...
| 类型 | 版本示例 | 必须验证 |
| --- | --- | --- |
| 启动器 | `launcher-v0.11.12` | 完整测试、安装/升级/卸载、登录、下载、修复、诊断上传、启动前授权 |
| API | `api-v0.16.0` | 数据库与论坛一致性备份、独立端口端到端验收、原子部署、健康/就绪、认证、旧域名与公网端口 |
| 客户端档案 | `profile-pvp-fabric-1.20.1-v1.0.0` | 清单签名、全对象哈希、干净安装、加载器与 Java、回滚 |
| Velocity 插件 | `velocity-authorizer-v0.2.0` | Java 测试、配置模式、目标映射；部署和重启分开记录 |
...
PVP `pvp-fabric-1.20.1` 使用 Java 17 与 Fabric `0.16.14`；基础和 NeoForge 1.21.11 档案使用 Java 21。发布记录必须保留这个运行时差异，不能因为客户端均从同一启动器进入而共用可写游戏目录。
...
`0.11.12` 首轮 2 至 3 人测试按
[`PRELAUNCH_PILOT_0.11.12.md`](PRELAUNCH_PILOT_0.11.12.md) 执行。该手册的“可以开始”

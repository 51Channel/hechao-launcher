...
- `2026-08-17` 生产 `LatestVersion=0.15.9`，最低支持版本保持 `0.12.3`。
- 安装包为 `61,995,538` 字节，SHA-256
  `88AE1869A47E313291AF0C60F8984BF14C1B5D0719291D72DDA254870487DE79`。
- 私有对象首次上传成功，重复发布校验后跳过；两轮签名读取 `200`、匿名读取 `403`，
  签名 URL 未进入 Git、文档、结构化证据或面向用户的输出。
- 认证会话确认 `0.15.8` 生成更新计划、`0.15.9` 不重复更新；API 签发入口和官网公开
  下载的长度、SHA-256 均与安装包一致。
- `0.15.8 -> 0.15.9` 隔离覆盖安装、全新安装、双轮卸载、设置与会话保留均通过。
- 更新元数据切换一次成功；API 保持 `0.33.1`，常驻 Publisher Agent 保持 `1.2.1`；
  Nginx、Minecraft、Velocity 和服控代理均未操作。

完整记录见 [`LAUNCHER_RELEASE_0.15.9.md`](LAUNCHER_RELEASE_0.15.9.md)。

## 0.15.10 生产快照

- `2026-08-24` 生产曾切换为 `LatestVersion=0.15.10`，最低支持版本保持 `0.12.3`；
- 安装包为 `61,997,588` 字节，SHA-256
  `6E3E449BA1FF59EF3B3AEA1EFD627F30A03E769BC57484033EA8D326AFCD4CC5`；
- 私有对象首次上传成功，重复发布校验后跳过；两轮签名读取 `200`、匿名读取 `403`；
- `0.15.9 -> 0.15.10` 覆盖安装、全新安装、双轮卸载、设置与会话保留通过；
- 认证会话确认旧版生成更新计划、当前版不重复更新，完整下载长度和 SHA-256 一致。

完整记录见 [`LAUNCHER_RELEASE_0.15.10.md`](LAUNCHER_RELEASE_0.15.10.md)。

## 0.15.12 生产快照

- `2026-08-24` 生产曾切换为 `LatestVersion=0.15.12`，最低支持版本保持 `0.12.3`；
- 安装包为 `61,998,219` 字节，SHA-256
  `ED3E6100652D81450ACF5392E584ECC04EEBA14E5EA2B65C60A6CCE98C5F0DB3`；
- DPAPI 会话改为完整、原子持久化；有效访问令牌启动时直接复用，临时网络错误不再清除
  登录态，只有刷新接口明确 `401` 才要求重新登录；
- 私有对象首次上传成功，重复发布校验后跳过；两轮签名读取 `200`、匿名读取 `403`；
- `0.15.10 -> 0.15.12` 覆盖安装、全新安装、双轮卸载、设置与会话保留通过；认证更新
  计划和完整下载通过；
- `0.15.11` 为构建元数据不合格的内部中间制品，通道已自动回滚，对象保留但不创建标签。
...
- 使用最终赫朝品牌图标统一 WPF、主程序、任务栏、安装器、卸载器和快捷方式，并完整继承
  `0.15.12` 的持久登录修复；
- 私有对象首次上传成功，重复发布校验后跳过；两轮签名读取 `200`、匿名读取 `403`；
- `0.15.12 -> 0.15.13` 覆盖安装、全新安装、双轮卸载、设置与会话保留通过；
- 认证会话确认 `0.15.12` 生成更新计划、`0.15.13` 不重复更新，完整下载长度和 SHA-256

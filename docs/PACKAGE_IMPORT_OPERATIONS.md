...
> 当前生产：API `0.28.4`、Publisher Agent `1.0.0`、owl5 与 owl9
...
> 删除。完整解决方案 `673/673`、API `285/285`、Publisher `39/39`、
...
- 已删除固定活动槽无法读取快速设置时，API 只对严格匹配的 `activity` 目标派生
  `4096 MiB` 部署上限；目录仍存在但设置读取失败、目标身份不匹配或部署能力关闭时
  继续拒绝。owl5 代理仍以本机 `8192 MiB` 配置做最终校验。

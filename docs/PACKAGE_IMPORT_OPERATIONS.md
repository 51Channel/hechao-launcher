...
> 源码候选：API `0.27.0`、Publisher Agent `1.0.0`、ServerControlAgent
> `0.3.0`
>
> 当前状态：功能源码、后台页面、无秘密配置模板和自动回滚路径已实现；尚未部署生产，
> 不代表整合包、客户端档案或活动服务端已经发布。完整解决方案 `622/622`、API
> `266/266`、Publisher `39/39`、ServerControlAgent `37/37`、Vitest `8/8`、
> Playwright `14/14` 和三个目标运行时发布均已通过。

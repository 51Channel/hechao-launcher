...
> 当前候选：API `0.32.0`、Publisher Agent `1.2.1`、owl5
> ServerControlAgent `0.7.0`、Velocity Authorizer `0.5.0`。
>
...
> `0.32.0 / 0.7.0 / 0.5.0` 将动态槽升级为独立端口和独立 Velocity 目标；生产发布与
> 最终测试数以对应发布记录为准。本能力不会自动启动 Minecraft，也不会改变
> Gray/Production。
...
- 服务端只允许部署到固定 `activity`，或从该安全模板创建且状态为 `Ready` 的动态槽。
  管理员按用途选择 `Activity`、`Survival`、`Pvp`、`Minigame`，对应 ID 前缀为
  `activity-*`、`survival-*`、`pvp-*`、`minigame-*`。
- 固定 `activity` 继续使用 `127.0.0.1:25568 / owl5-activity-slot`，只用于共享该入口的
  活动替换服。动态槽从 `25600-25611` 自动分配未占用端口，Velocity 目标等于槽 ID，
  冲突组为空；不同动态槽以及固定生存/PVP/小游戏服可以同时运行。
- 动态槽拥有独立 `E:\HechaoActivitySlots\<serverId>` 目录和
  `Hechao-Server-<serverId>` 计划任务。新槽默认停止、隐藏且不进入玩家目录；部署标记
  有效前返回 `DEPLOYMENT_REQUIRED`，不能启动空槽。
- “仅发布并入库”不要求 owl5 代理在线，也不会读取、停止或覆盖当前活动槽；只有选择
  “立即部署所选槽”或稍后从企划页部署时，目标才必须停止且不能有其他活动操作。流程
  不会为了导入自动停止冲突服，也不会在部署成功后自动启动 Minecraft。
- 已删除并完成清理的部署目录不会出现在普通服控列表，但整合包页会读取保留的受控槽
...
3. 管理员填写档案 ID、显示名、语义版本、最低称号、最大内存、世界保留策略和目录
   ...
   所需槽不存在时，可选择生存、活动、PVP 或小游戏类型，再输入对应前缀的 ID 后缀、
   名称、原因和精确确认文本创建。API 自动分配独立端口并先写入 `Provisioning`；代理
   完成目录、固定文件快照和计划任务后才变为 `Ready` 并自动选中。
...
API 与 owl5 生产配置均限制为 `12` 个 `Provisioning / Ready` 动态槽，对应
`25600-25611` 共 12 个独立端口；`Failed` 槽不占用该额度。管理员不能指定任意 VPS
路径、端口、任务名、冲突组或启动命令。代理只从固定模板派生安全配置，并把成功状态原子保存到
`dynamic-deployment-slots.json`。若目录、任务、快照或状态安装任一步失败或代理取消，
本轮创建的受控资源会回滚；既有目录或既有计划任务一律拒绝覆盖。

槽类型只描述业务用途。运行隔离由独立端口、槽自身 Velocity 目标和空冲突组保证；
Velocity Authorizer 只接受 API 返回的 `127.0.0.1` / `::1` 动态地址，拒绝注册任意外部
后端，也不会覆盖地址不一致的既有目标。
...
owl5 当前正式代理为 `0.6.0`，独立槽候选为 `0.7.0`。目录心跳、快捷设置和目录切换使用同一目标级访问门闩；

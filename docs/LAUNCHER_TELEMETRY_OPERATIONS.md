...
> 启动器源码候选：`0.11.13`
>
> API 当前生产：`0.19.0-20260727T005013Z`（遥测行为自 `0.18.0` 起保持兼容）
>
> 当前状态：服务端遥测已生产部署；启动器 `0.11.13` 待构建和私有灰度
...
还需在启动器 `0.11.13` 私有灰度后确认真实客户端产生 `LauncherStarted`，并在真实管理员
完成 MFA 后打开“运行数据”切换三个时间窗口。这两项属于真实账号验收，不影响 API
当前生产状态。

...
> API 源码候选：`0.18.0`
>
> 当前状态：代码、自动测试和隔离生产副本验收已完成；生产 API 仍为 `0.17.0`
...
3. 使用真实启动器登录后产生一条 `LauncherStarted`，重复提交不重复计数。
4. 真实管理员完成 MFA 后打开“运行数据”，切换三个时间窗口。
5. `hechao.world`、`api.hechao.world`、旧目录、下载和 Velocity 授权回归正常。

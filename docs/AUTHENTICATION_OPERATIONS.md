...
> 当前客户端源码版本：`0.8.1`，API 源码版本：`0.9.0`
> 当前生产状态：API `0.9.0` 与赫朝账号链路已部署；启动器 `0.8.1` 尚未分发，Velocity 仍为 `monitor`，目录强制登录尚未启用
...
7. Client ID 已写入 `0.8.1` 客户端候选；环境变量 `HECHAO_MICROSOFT_CLIENT_ID` 仍可用于内部覆盖测试。

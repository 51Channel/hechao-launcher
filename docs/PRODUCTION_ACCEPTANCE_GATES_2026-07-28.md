...
### 3.4 PVP 真实 Velocity 路由：首次失败已修复，仍待复测
...
修复后的启动日志同时加载 FabricProxy-Lite `2.6.0` 与 CrossStitch，但当前 PVP
端口未监听，尚不能声明修复通过。仍须在管理员控制的持久开服窗口显式验证统一入口、
UUID/名称/皮肤/权限、直连拒绝、`/hub` 和断线重连。完成前不得把 PVP 路由记为
生产通过，也不得切换 Velocity `enforce`。
...
单账号真实路由和正常退出；PVP 修复后进服、两档案修复/回滚和多人样本仍需按以下
顺序补齐：
...
1. 在持久开服窗口完成 PVP 修复后的显式启动和真实拒绝/允许路径；Activity 单账号路由已通过。

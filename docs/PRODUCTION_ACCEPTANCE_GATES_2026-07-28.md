...
### 3.4 PVP 真实 Velocity 路由：服务端与档案已就绪，进服待验收

FabricProxy-Lite `2.6.0`、modern forwarding 和指标代理均已加载，PVP 监听正常。
正确赫朝客户端已完成 PVP Fabric `1.0.0` 档案、独立 Java 17 和全部清单文件安装；
初始化日志显示客户端已使用正版身份向统一入口建立连接，且没有崩溃报告，但尚未把这次
初始化扩大解释为完整路由验收。仍必须显式启动并验证统一入口、UUID/名称/皮肤/权限、
...
个清单文件无缺失，只有两个客户端运行时可变配置与发布对象不同。Activity/PVP 的
显式 Launch/GameExit、真实路由、修复与回滚样本仍需按以下顺序补齐：
...
1. 用正确赫朝客户端完成 Activity/PVP 专用档案的显式启动和真实拒绝/允许路径；生产安装已完成。

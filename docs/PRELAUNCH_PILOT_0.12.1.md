...
> 已部署并通过基础健康检查后执行。
>
> 当前状态：自动发布、特殊路径、Activity 冷/热启动、安装版单账号真实进服、
> 同/跨档案三轮切换、启动器重启接管以及正常/异常退出后的 fresh grant 恢复已于
> 2026-07-30 完成；维护与关闭目标的运行中拦截也已通过，四级账号、离线/API
> 故障拒绝与多人项目仍需要真实玩家执行。
...
- [x] 同档案连续执行三轮 fresh grant 重进；当前无两个不同在线目标共用同一档案，每轮均验证旧进程完全退出后才启动新进程。
- [x] 跨档案换服连续执行三轮；`4,261` 个 200 ms 采样中 Minecraft Java 进程峰值为 `1`。
- [x] 启动器关闭再打开后能重新附着仍在运行的游戏，并正确恢复 `base-1.21.11` / `survival2`。
- [x] 目标服维护或关闭时主操作禁用；已有 Activity 进程保持不变，不创建第二个游戏进程，也不回退到大厅。
- [ ] 目标服离线或当前账号无权限时不启动游戏，也不回退到大厅。
- [ ] API 暂时不可用时首次连接硬拒绝；恢复后重新申请授权可正常直达目标。
- [x] Lobby 只监听 `127.0.0.1:25566` 且公网端口不可达；Lobby Guard 与空白名单保持生效。大厅八个玩家 Skript 已在线禁用，owl5 后端、Velocity 和 owl9 恐怖整蛊均无活动的 `/hub`、`/lobby`、NPC 命令或插件转服路径。
- [x] 游戏正常退出与异常退出状态、诊断包和运行遥测均正确；强制结束本地客户端后显示“游戏异常退出”，清理持久状态，并用新授权重新进入 `survival2`。

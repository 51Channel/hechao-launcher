...
- `Adopt-MinecraftServerRuntime.ps1` 已在 owl5 的 Survival2、Activity、大厅和
  owl9 恐怖整蛊运行实例上完成 `-WhatIf` 祖先进程验证，结果均为
  `verified-only`，没有写标记或执行游戏服动作。
...
实时盘点另确认 owl5 的 `ActivityNeoForge`、`FanStreet`、`Yugong` 共用
`25568`，`Survival2` 与 `DollNight` 共用 `25565`。对应无密钥生产白名单和机器
证据已写入仓库；代理仍未安装，API 服控仍保持关闭。

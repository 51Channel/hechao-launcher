...
> 更新时间：`2026-08-06`
>
> 当前生产：启动器 `0.14.2`、API `0.28.1`、Publisher Agent `1.0.0`、
...
| API `0.28.1` | 已完成 | Vue 十页、迁移 24/24、整合包任务编排和白名单服务端文件删除均已部署；已完成删除和清理的目标从日常服控列表隐藏，重新部署后自动恢复。数据库与配置备份、原子切换、内外网健康和生产状态回归通过，见 [`API_RELEASE_0.28.1.md`](API_RELEASE_0.28.1.md) |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| API 与代理版本 | 已完成 | API `0.28.1`、owl5 与 owl9 代理 `0.4.0` 已部署；API 服务正常，代理在线状态和删除能力按实时心跳故障关闭，见 [`API_RELEASE_0.28.1.md`](API_RELEASE_0.28.1.md) 与 [`SERVER_CONTROL_AGENT_RELEASE_0.4.0.md`](SERVER_CONTROL_AGENT_RELEASE_0.4.0.md) |
...
| 生产进程隔离 | 已完成 | `0.28.1` 只重启 API，没有向 Minecraft、Velocity、Publisher 或两台服控代理发送控制命令。生产已有 `activity/fanstreet/yugong` 三个成功删除记录；三者目录不存在、清理完成且无活动操作，当前概览显示其余 6 个目标 |
...
| 下载量、失败率与版本分布 | 已实现待生产验收 | 当前 API `0.28.1` 保留迁移 15、30 天留存、幂等批次和后台“运行数据”，并对客户端下载失败率建立统一告警；真实基础、Activity 与恐怖整蛊安装样本已完成，仍需真实回滚、完整 Launch/GameExit 和多人样本 |
...
| 账号、设备会话与 Minecraft UUID 封禁 | 已实现待生产验收 | 功能自 API `0.16.0` 上线并保留在当前 `0.28.1`；账号停用/恢复、单设备和全部会话撤销、UUID 定时封禁、论坛既有 Cookie 联动、并发保护、Velocity/目录拒绝及审计均通过隔离与生产链路验收。真实管理员 MFA 已登记，仍待在生产页面逐项操作验收 |
...
| 删除停用服务端文件 | 已完成 | API `0.28.1` 与两台代理 `0.4.0` 已生产部署；后台危险确认、CSRF、原因、审计、停止态、命令互斥、路径边界、同卷移出、清理重试和幂等均已验收。生产已有 `activity/fanstreet/yugong` 三个成功删除记录；完成清理后不再显示在日常服控列表，审计、代理配置和外置备份保留，重新部署后自动恢复。见 [`API_RELEASE_0.28.1.md`](API_RELEASE_0.28.1.md) |
...
11. [生产只读验收已完成] 服控 API `0.28.1`、owl5 与 owl9 代理 `0.4.0` 已启用；

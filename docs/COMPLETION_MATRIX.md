...
> 更新时间：`2026-08-06`
>
> 当前生产：启动器 `0.14.2`、API `0.28.0`、Publisher Agent `1.0.0`、
...
| API `0.28.0` | 已完成 | Vue 十页、迁移 24/24、整合包任务编排和白名单服务端文件删除均已部署；数据库与配置备份、原子切换、内外网健康、代理状态和零真实删除回归通过，见 [`API_RELEASE_0.28.0.md`](API_RELEASE_0.28.0.md) |
...
| 自动测试 | 已完成 | 当前基线为完整解决方案 `666/666`、API `278/278`、Publisher `39/39`、ServerControlAgent `51/51`、Vue Vitest `8/8`、Playwright `15/15`；Velocity、Lobby、LuckPerms 和指标代理保持各自既有正式基线 |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| API 与代理版本 | 已完成 | API `0.28.0`、owl5 与 owl9 代理 `0.4.0` 已部署；API 服务正常，代理在线状态和删除能力按实时心跳故障关闭，见 [`API_RELEASE_0.28.0.md`](API_RELEASE_0.28.0.md) 与 [`SERVER_CONTROL_AGENT_RELEASE_0.4.0.md`](SERVER_CONTROL_AGENT_RELEASE_0.4.0.md) |
...
| 生产进程隔离 | 已完成 | API/代理升级前后 owl5 五个、owl9 一个 Java PID 不变；最终 9 个目录均存在、清理残留为 `0`，删除操作、进行中操作和命令均为 `0`，本次没有执行真实删除 |
...
| 下载量、失败率与版本分布 | 已实现待生产验收 | 当前 API `0.28.0` 保留迁移 15、30 天留存、幂等批次和后台“运行数据”，并对客户端下载失败率建立统一告警；真实基础、Activity 与恐怖整蛊安装样本已完成，仍需真实回滚、完整 Launch/GameExit 和多人样本 |
...
| 账号、设备会话与 Minecraft UUID 封禁 | 已实现待生产验收 | 功能自 API `0.16.0` 上线并保留在当前 `0.28.0`；账号停用/恢复、单设备和全部会话撤销、UUID 定时封禁、论坛既有 Cookie 联动、并发保护、Velocity/目录拒绝及审计均通过隔离与生产链路验收。真实管理员 MFA 已登记，仍待在生产页面逐项操作验收 |
...
| 删除停用服务端文件 | 已完成（零真实删除） | API `0.28.0` 与两台代理 `0.4.0` 已生产部署；后台危险确认、CSRF、原因、审计、停止态、命令互斥、路径边界、同卷移出、清理重试和幂等均已验收。当前只证明能力安全上线，未为验收删除任何真实服务端，见 [`evidence/SERVER_DIRECTORY_DELETION_PRODUCTION_DEPLOYMENT_2026-08-06.json`](evidence/SERVER_DIRECTORY_DELETION_PRODUCTION_DEPLOYMENT_2026-08-06.json) |
...
11. [生产只读验收已完成] 服控 API `0.28.0`、owl5 与 owl9 代理 `0.4.0` 已启用；
    9 个目标、2 个代理、5 个删除能力位与冲突组均已核对，部署前后游戏 PID 不变，
    未执行真实删除。下一步只在专用无玩家目标完成真实启停、失败取消、终端和快捷设置验收。

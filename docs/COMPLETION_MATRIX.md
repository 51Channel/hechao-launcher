...
> 更新时间：`2026-07-30`
>
> 当前生产：启动器 `0.12.1`、API `0.22.0-20260729T144953Z`、
...
| 项目 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| 启动器 `0.12.1` | 已部署待外部验收 | 私有 OSS 已发布；匿名 `403`、两次签名回读 `200`、长度和 SHA-256 复验通过，见 [`LAUNCHER_RELEASE_0.12.1.md`](LAUNCHER_RELEASE_0.12.1.md) |
...
| Windows 安装、覆盖升级与卸载 | 已完成 | `0.12.0 -> 0.12.1`、隔离干净安装、双轮卸载、设置与 DPAPI 会话保留、ProductVersion 和运行中启动器保护均通过 |
| 私有 OSS 发布 | 已完成 | `0.12.1` 不可变对象已发布；重复发布校验后跳过，私有签名 URL 未进入文档或日志 |
| 自动测试 | 已完成 | 当前开发分支 .NET `384/384`，Velocity `26/26`，Lobby Guard `3/3`，LuckPerms 等级代理 `4/4`，Paper/Purpur 指标代理 `2/2` |
| 2 至 3 人真实灰度 | 外部验收 | 待按 `0.12.1` 单进程切服与 Lobby 隔离清单执行 |
...
| Minecraft 退出状态刷新 | 已完成 | `0.11.16` 在当前档案正常或异常退出后恢复状态和主操作，并防止旧进程延迟退出事件覆盖同档案的新进程；自动测试、本机升级及 `0.12.1` Activity 安装版真实启动/退出码 `0` 回归均通过 |
| 每档案 `.minecraft` 和 Java 17/21 | 已完成 | 自动 Java、自定义 Java、NTFS 短工作目录、受管 Java/原生库独立兼容映射和真实进程构建 |
...
| Activity NeoForge 重复启动稳定性 | 已完成 | `0.12.1` 修复含 U+200C 数据根目录下的 Windows 路径兼容：游戏工作目录使用 NTFS 8.3 短路径，受管 Java 与版本 `natives` 分别使用安全映射；冷、热两轮真实 NeoForge 进程均越过 Jar-in-Jar 模组发现并进入 Meccha 加载，四个原生目录属性和 `user.dir` 经 `jcmd` 复验。安装版启动器另完成正版会话、连接 `mc.hehe11.fun`、进入 Activity 世界、正常退出码 `0` 和零残留进程验收；断线重连与多人行为仍随灰度执行 |
| 安装失败自动恢复当前版本 | 已完成 | 原子切换与失败回滚测试 |
| 玩家主动回滚到上一版本 | 已完成 | 同锁安全克隆、玩家数据保留、确认界面、运行中保护、Java 复验及失败恢复测试 |
| Fabric 启动 | 已完成 | 基础与恐怖整蛊档案 |
| NeoForge 启动 | 已完成 | 活动档案 `21.11.42`；`0.12.1` 冷、热真实进程启动与原生库路径验收通过 |
...
| 启动器唯一切服与单 Minecraft 进程 | 已部署待外部验收 | `0.12.1` 已发布并实现持久进程接管、安全退出/超时收口、退出完成后才重新授权、全局单进程和全部 Lobby 目录过滤；仍需同档案/跨档案各三轮真实切换与启动器重启接管 |
...
8. 使用四级真实账号复测所有目标的直接进入、同档案/跨档案三轮切换、启动器重启接管、断线恢复与 API 故障，再切换 Velocity `enforce`。

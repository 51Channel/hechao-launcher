...
| Activity NeoForge 重复启动稳定性 | 已完成自动验收 | `0.12.1` 修复含 U+200C 数据根目录下的 Windows 路径兼容：游戏工作目录使用 NTFS 8.3 短路径，受管 Java 与版本 `natives` 分别使用安全映射；冷、热两轮真实 NeoForge 进程均越过 Jar-in-Jar 模组发现并进入 Meccha 加载，四个原生目录属性和 `user.dir` 经 `jcmd` 复验，进程自动收口且无残留。真实玩家完整进服仍随单账号灰度执行 |

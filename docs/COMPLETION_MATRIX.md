...
| Activity/PVP 生产档案安装 | 已完成 | 正确赫朝启动器已安装 Activity `1.0.10` 与 PVP `1.0.0`，独立 Java 21/17 和目录均存在；清单 `8,503` 项无缺失，`8,501` 项逐字节一致，另两项仅为客户端运行时可变配置。Activity 单账号真实路由与正常退出已通过；PVP 首次路由暴露后端包解码问题，CrossStitch 修复已部署但仍待复测 |
...
| 能力 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| Velocity 异步授权、首次目标与客户端兼容 | 已实现待生产验收 | API `0.20.2` + 插件 `0.3.0` 以 `monitor` 运行；生产兼容矩阵 `8/8`，基础档案 Lobby/Survival1/Survival2 与 Activity 单账号真实转服通过；PVP 服务端兼容修复后的真实进服仍待验收 |
| PVP Fabric 后端 modern forwarding | 已部署待生产验收 | owl9 已加载 FabricProxy-Lite `2.6.0` 与官方 CrossStitch `0.1.6`，部署前完成 `save-all flush`、优雅停服和独立备份；上次启动完成但当前端口未监听，需在管理员控制的持久开服窗口重启后验证真实路由、身份转发和直连拒绝 |

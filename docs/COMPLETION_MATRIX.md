...
> 更新时间：`2026-08-08`
>
> 当前生产：启动器 `0.15.1`、API `0.29.0`、Publisher Agent `1.2.1`、
...
| 项目 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| 启动器 | 已发布，更新通道已开放 | `0.15.1` 已发布到私有 OSS；两轮签名回读 `200`、匿名读取 `403`，真实登录会话确认 `0.15.0` 可取得更新计划并完整下载，见 [`LAUNCHER_RELEASE_0.15.1.md`](LAUNCHER_RELEASE_0.15.1.md) |
...
| Windows 安装、覆盖升级与卸载 | 已完成 | `0.15.0 -> 0.15.1` 隔离覆盖升级、全新安装、双轮卸载、设置与会话保留均通过；正式安装进程未被替代关闭 |
| 私有 OSS 发布 | 已完成 | `0.15.1` 不可变对象已发布；第二轮重复发布校验后跳过，匿名读取 `403`，两轮签名回读 `200`，私有签名 URL 未进入 Git、文档或终端记录 |
| 自动测试 | 已完成 | `0.15.1` 完整解决方案 `707/707`、Launcher `222/222`、Publisher `55/55`；API `0.29.0` 为 API `294/294`、完整解决方案 `700/700`、Vitest `8/8`、Playwright `20/20` |
| 2 至 3 人真实灰度 | 外部验收 | 待按 `0.15.1` 单进程切服与 Lobby 隔离清单执行 |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| 正式版本与制品 | 已完成 | `0.15.1` 已发布私有 OSS；EXE 与安装包版本、大小、SHA-256、`NotSigned` 状态见 [`LAUNCHER_RELEASE_0.15.1.md`](LAUNCHER_RELEASE_0.15.1.md) |
| 自动测试 | 已完成 | 完整解决方案 `707/707`，启动器 `222/222`，Publisher `55/55` |
| 官网同源活动月历 | 已完成 | 固定六周、周一开头、月份切换、今天、跨日活动、待排期和日期详情已上线；官网与启动器读取同一 API 排期，零活动时仍显示完整月历 |
| 服务器主页三栏改版 | 已完成 | 宽导航、真实服务器目录、当前服务器主视区、真实公告、近期活动和 Java/内存/目录快捷设置已上线；未引入伪造数据或玩家端管理入口 |
...
| 生产更新通道 | 已完成 | `LatestVersion=0.15.1`、`MinimumSupportedVersion=0.12.3`；真实会话与 API 签发下载链通过，公网健康与就绪端点均为 `200`，服务 `NRestarts=0`；一次错误契约验收已真实触发并完成自动回滚 |
...
| 快捷设置与维护能力 | 源码候选 | Java、内存和游戏目录保留首屏快捷设置；下方详情区已移除，回滚、修复、删除、设置和 Java 模式集中到主按钮右侧三点菜单，命令与确认逻辑不变 |
| 自动与视觉验证 | 已发布（`0.15.1`） | Release 构建 `0` 警告、`0` 错误；启动器 `222/222`、完整解决方案 `707/707`；`1500 x 860` 与 `1060 x 640` WPF 截图无横向溢出、重叠或操作裁切；Impeccable detector 无发现 |
...
| 启动器本体自更新 | 已完成 | 每个进程启动并恢复登录态后检查；私有 OSS 断点下载、长度与 SHA-256 复验、临时更新器静默覆盖、失败保留及重试入口均已发布。生产 `LatestVersion=0.15.1`；真实会话确认 `0.15.0` 可更新且 `0.15.1` 不重复提示，见 [`LAUNCHER_SELF_UPDATE_OPERATIONS.md`](LAUNCHER_SELF_UPDATE_OPERATIONS.md) |

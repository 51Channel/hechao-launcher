...
> 更新时间：`2026-08-17`
>
> 当前生产：启动器 `0.15.9`、API `0.33.1`、LuckPerms Tier Agent `0.1.3`、Publisher Agent `1.2.1`、
...
| API `0.33.1` | 已完成 | 保留迁移 031、四类独立槽、动态回环授权、全命令控制台和目录完整性过滤；补齐经济商品目录生产合同。内外网健康、数据库 `31/31` 和回滚均已验收，见 [`API_RELEASE_0.33.1.md`](API_RELEASE_0.33.1.md) |
| 独立生存/活动/PVP/小游戏槽 | 已完成 | API `0.33.1`、owl5/owl9 代理 `0.7.2`、Velocity Authorizer `0.5.0` 已生产部署；未发布 Production 客户端的独立槽继续保留在后台，但不会进入普通玩家实时目录 |
...
| Windows 安装、覆盖升级与卸载 | 已完成 | `0.15.8 -> 0.15.9` 隔离覆盖升级、全新安装、双轮卸载、设置与会话文件保留均通过；验收开始时的既有正式启动器进程未被替代关闭 |
| 私有 OSS 发布 | 已完成 | `0.15.9` 不可变对象已发布；第二轮重复发布校验后跳过，匿名读取 `403`，两轮签名回读 `200`，私有签名 URL 未进入 Git、文档或终端记录 |
| 自动测试 | 已完成 | 当前分支完整解决方案 `806` 项通过、`1` 项环境测试跳过，其中 API `359/359`、Agent `76/76`、Launcher `234/234`、Distribution `47/47`；改动格式、PowerShell 7 和 `git diff --check` 通过 |
| 2 至 3 人真实灰度 | 外部验收 | 待按 `0.15.9` 单进程切服、活动权限拆分与 Lobby 隔离清单执行 |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| 正式版本与制品 | 已完成 | `0.15.9` 已发布私有 OSS；EXE 与安装包版本、大小、SHA-256、`NotSigned` 状态见 [`LAUNCHER_RELEASE_0.15.9.md`](LAUNCHER_RELEASE_0.15.9.md) |
| 自动测试 | 已完成 | 完整解决方案 `806` 项通过、`1` 项环境测试跳过；启动器 `234/234`、API `359/359`、Distribution `47/47`，其余组件测试全部通过 |
...
| 生产更新通道 | 已完成 | `LatestVersion=0.15.9`、`MinimumSupportedVersion=0.12.3`；公网健康、就绪、公开元数据、官网下载网关和认证会话更新计划通过，服务 `NRestarts=0` |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| API 与代理版本 | 已完成 | API `0.33.1`、owl5/owl9 代理 `0.7.2` 已部署；全命令合同继续来自 `0.32.1`，目录档案完整性修复见 [`API_RELEASE_0.32.2.md`](API_RELEASE_0.32.2.md)，经济商品目录修复见 [`API_RELEASE_0.33.1.md`](API_RELEASE_0.33.1.md)，代理记录见 [`SERVER_CONTROL_AGENT_RELEASE_0.7.2.md`](SERVER_CONTROL_AGENT_RELEASE_0.7.2.md) |
...
| 启动器本体自更新 | 已完成 | 每个进程启动并恢复登录态后检查；私有 OSS 断点下载、长度与 SHA-256 复验、临时更新器静默覆盖、失败保留及重试入口均已发布。生产 `LatestVersion=0.15.9`；认证会话确认 `0.15.8` 生成更新计划、`0.15.9` 不重复更新，见 [`LAUNCHER_SELF_UPDATE_OPERATIONS.md`](LAUNCHER_SELF_UPDATE_OPERATIONS.md) |
...
| 下载量、失败率与版本分布 | 已实现待生产验收 | 当前 API `0.33.1` 保留迁移 15、30 天留存、幂等批次和后台“运行数据”，并对客户端下载失败率建立统一告警；真实基础、Activity 与恐怖整蛊安装样本已完成，仍需真实回滚、完整 Launch/GameExit 和多人样本 |
...
| 删除停用服务端文件 | 已完成 | 当前 API `0.33.1` 与双机代理 `0.7.2` 保留完整删除合同；后台危险确认、CSRF、原因、审计、停止态、命令互斥、路径边界、同卷移出、清理重试和幂等均已验收。生产已有 `activity/fanstreet/yugong` 三个成功删除记录；完成清理后不再显示在日常服控列表，审计、代理配置和外置备份保留，整合包页仍可重新部署固定活动槽。原始功能证据见 [`API_RELEASE_0.28.3.md`](API_RELEASE_0.28.3.md) |

...
> 更新时间：`2026-09-04`
>
> 当前生产：启动器 `0.15.14`、API `0.38.1`、LuckPerms Tier Agent `0.1.3`、Publisher Agent `1.2.1`、
...
| API `0.38.1` | 已完成 | 保留 `0.38.0` 的整合包 Java 主版本传递，并修复旧管理后台登录页无法感知其他标签页已建立会话的问题；数据库保持 `35/35`，生产健康、就绪、安全边界、深层路由恢复和回滚均已验收，见 [`API_RELEASE_0.38.1.md`](API_RELEASE_0.38.1.md) |
...
| 独立生存/活动/PVP/小游戏槽 | 已完成 | API `0.38.1`、owl5 Agent `0.8.2`、owl9 Agent `0.7.2` 与 Velocity Authorizer `0.5.0` 已生产部署；owl5 动态启动任务和控制台桥已改为无人登录可运行的 `S4U`；未发布 Production 客户端的独立槽继续保留在后台，但不会进入普通玩家实时目录 |
...
| 整合包导入 | 已完成（真实 Java 8 包 Test-only） | API `0.38.1`、Publisher `1.2.1` 与 owl5 Agent `0.8.2` 保留商业街 Forge `1.12.2` 标准包分析、客户端 Test-only 发布、独立 Minigame 部署及无人登录真实启动；协议 `340`、`SELFTEST PASS`、控制台 `list` 和零启动错误通过。目录仍隐藏并保持 `Closed`，Gray/Production 未分配；真人进服及玩法仍按活动单独验收，见 [`COMMERCIAL_STREET_PACKAGE_1.0.0.md`](COMMERCIAL_STREET_PACKAGE_1.0.0.md) |
...
| 自动测试 | 已完成 | API `383/383`、Agent `84/84`、AdminWeb Vitest `19/19`、Playwright `35/35`、完整解决方案 `838` 项通过，`1` 项外部 PostgreSQL 条件测试跳过；Release 构建 `0` 警告、`0` 错误，证据 JSON、PowerShell 7 脚本和 `git diff --check` 通过 |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| API 与代理版本 | 已完成 | API `0.38.1`、owl5 Agent `0.8.2`、owl9 Agent `0.7.2` 已部署；后台会话恢复见 [`API_RELEASE_0.38.1.md`](API_RELEASE_0.38.1.md)，多 Java 与旧 Forge 设置兼容见 [`API_RELEASE_0.38.0.md`](API_RELEASE_0.38.0.md) 和 [`SERVER_CONTROL_AGENT_RELEASE_0.8.1.md`](SERVER_CONTROL_AGENT_RELEASE_0.8.1.md)，owl5 无人登录启动修复见 [`SERVER_CONTROL_AGENT_RELEASE_0.8.2.md`](SERVER_CONTROL_AGENT_RELEASE_0.8.2.md) |
...
| 下载量、失败率与版本分布 | 已实现待生产验收 | 当前 API `0.38.1` 保留迁移 15、30 天留存、幂等批次和后台“运行数据”，并对客户端下载失败率建立统一告警；真实基础、Activity 与恐怖整蛊安装样本已完成，仍需真实回滚、完整 Launch/GameExit 和多人样本 |
| 经济行情、财富分布、玩家市场与单品 K 线 | 已部署待真实交易验收 | API `0.35.0+` 已上线玩家挂单、购买、下架、待领取和模糊搜索；当前 `0.38.1` 同时保留 24 小时、7/30/90 天货币供给、财富分布、官方回收和真实成交 K 线。事务、幂等与余额由服务端裁决；双真人账号的完整交易、断线、背包竞争和余额守恒仍待 Test 验收 |
...
| 删除停用服务端文件 | 已完成 | 当前 API `0.38.1`、owl5 Agent `0.8.2` 与 owl9 Agent `0.7.2` 保留完整删除合同；后台危险确认、CSRF、原因、审计、停止态、命令互斥、路径边界、同卷移出、清理重试和幂等均已验收。完成清理后不再显示在日常服控列表，审计、代理配置和外置备份保留；原始功能证据见 [`API_RELEASE_0.28.3.md`](API_RELEASE_0.28.3.md) |
...
11. [生产验收已完成] 服控 API `0.38.1`、owl5 Agent `0.8.2` 与 owl9 Agent `0.7.2` 已启用；

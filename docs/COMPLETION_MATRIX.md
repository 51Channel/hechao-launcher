...
> 更新时间：`2026-08-15`
>
> 当前生产：启动器 `0.15.8`、API `0.32.0`、LuckPerms Tier Agent `0.1.3`、Publisher Agent `1.2.1`、
> owl5 ServerControlAgent `0.7.0`、owl9 ServerControlAgent `0.4.0`、
...
| API `0.32.0` | 已完成 | 上线迁移 030、四类独立部署槽和动态回环后端授权；当前 release、数据库 `30/30`、内外网健康、后台静态资源与回滚均已验收，见 [`API_RELEASE_0.32.0.md`](API_RELEASE_0.32.0.md) |
| 独立生存/活动/PVP/小游戏槽 | 已完成 | API `0.32.0`、owl5 代理 `0.7.0`、Velocity Authorizer `0.5.0` 已生产部署；工业季为停止的 `Survival / 25600 / activity-survival`，固定活动服继续运行于 `25568`，两者不再共享冲突组。生产自动验收没有启动工业季，真实同时开服仍按需要单独安排 |
...
| 自动测试 | 已完成 | 当前分支完整解决方案 `748/748`，其中 API `326/326`、Launcher `229/229`；其余组件测试继续通过。全仓格式检查仍只命中未修改的既有 `MinecraftRunningStateStoreTests.cs` 空格问题 |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| 正式版本与制品 | 已完成 | `0.15.8` 已发布私有 OSS；EXE 与安装包版本、大小、SHA-256、`NotSigned` 状态见 [`LAUNCHER_RELEASE_0.15.8.md`](LAUNCHER_RELEASE_0.15.8.md) |
| 自动测试 | 已完成 | 完整解决方案 `748/748`，启动器 `229/229`，API `326/326`，Publisher `55/55`，其余组件测试全部通过 |
...
| 生产更新通道 | 已完成 | `LatestVersion=0.15.8`、`MinimumSupportedVersion=0.12.3`；公网健康、就绪、公开元数据、官网下载网关和认证会话更新计划通过，服务 `NRestarts=0` |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| API 与代理版本 | 已完成 | API `0.32.0`、owl5 代理 `0.7.0`、owl9 代理 `0.4.0` 已部署；API 服务正常，代理在线状态、独立槽和删除能力按实时心跳故障关闭，见 [`API_RELEASE_0.32.0.md`](API_RELEASE_0.32.0.md) 与 [`SERVER_CONTROL_AGENT_RELEASE_0.7.0.md`](SERVER_CONTROL_AGENT_RELEASE_0.7.0.md) |
| 目录与物理服状态同步 | 已完成 | `Online` 只作为管理员开放策略；同名服控目标在线时开放、停止时自动关闭、服控失联时故障关闭。目录删除并完成清理后，目标从服控概览隐藏；重新部署目录并恢复心跳后自动出现。`dollnight` 显式维护保持优先 |
| 代理心跳与命令隔离 | 已完成 | 心跳和命令使用独立循环；owl5 `0.7.0` 与 owl9 `0.4.0` 保留循环自恢复，并用目标级门闩隔离目录心跳、快捷设置、整合包切换和文件删除。独立槽发布中固定活动服和大厅 PID 未变化 |
...
| 启动器本体自更新 | 已完成 | 每个进程启动并恢复登录态后检查；私有 OSS 断点下载、长度与 SHA-256 复验、临时更新器静默覆盖、失败保留及重试入口均已发布。生产 `LatestVersion=0.15.8`；认证会话确认 `0.15.7` 生成更新计划、`0.15.8` 不重复更新，见 [`LAUNCHER_SELF_UPDATE_OPERATIONS.md`](LAUNCHER_SELF_UPDATE_OPERATIONS.md) |
...
| 下载量、失败率与版本分布 | 已实现待生产验收 | 当前 API `0.32.0` 保留迁移 15、30 天留存、幂等批次和后台“运行数据”，并对客户端下载失败率建立统一告警；真实基础、Activity 与恐怖整蛊安装样本已完成，仍需真实回滚、完整 Launch/GameExit 和多人样本 |
...
| 删除停用服务端文件 | 已完成 | 当前 API `0.32.0`、owl5 代理 `0.7.0` 与 owl9 代理 `0.4.0` 保留完整删除合同；后台危险确认、CSRF、原因、审计、停止态、命令互斥、路径边界、同卷移出、清理重试和幂等均已验收。生产已有 `activity/fanstreet/yugong` 三个成功删除记录；完成清理后不再显示在日常服控列表，审计、代理配置和外置备份保留，整合包页仍可重新部署固定活动槽。原始功能证据见 [`API_RELEASE_0.28.3.md`](API_RELEASE_0.28.3.md) |
...
11. [生产验收已完成] 服控 API `0.32.0`、owl5 代理 `0.7.0`、owl9 代理 `0.4.0` 已启用；

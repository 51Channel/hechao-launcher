...
> 更新时间：`2026-07-30`
>
> 当前生产：启动器 `0.12.2`、API `0.22.0-20260729T144953Z`、
...
| 项目 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| 启动器 `0.12.2` | 已部署待外部验收 | 私有 OSS 已发布；匿名 `403`、两次签名回读 `200`、长度和 SHA-256 复验通过，安装版 Activity 真实进服和退出码 `0` 已验收，见 [`LAUNCHER_RELEASE_0.12.2.md`](LAUNCHER_RELEASE_0.12.2.md) |
...
| Minecraft 退出状态刷新 | 已完成 | `0.11.16` 在当前档案正常或异常退出后恢复状态和主操作，并防止旧进程延迟退出事件覆盖同档案的新进程；`0.12.2` 另保存独立进程号以可靠清理过期运行状态，自动测试、本机升级及 Activity 安装版退出码 `0` 回归均通过 |
...
| Activity NeoForge 重复启动稳定性 | 已完成 | `0.12.2` 在既有短工作目录和安全映射上最终规范 `java.library.path`、`org.lwjgl.librarypath`、JNA、LWJGL 解压和 Netty 五个原生目录属性；真实安装版全部唯一指向无 U+200C 的安全目录，完成正版会话、NeoForge/LWJGL/Meccha 加载、连接 `mc.hehe11.fun`、进入 Activity 世界、正常退出码 `0` 和零残留进程验收，未再出现 `Can't find dependent libraries`。见 [`evidence/ACTIVITY_NEOFORGE_NATIVE_PATH_RECOVERY_2026-07-30.json`](evidence/ACTIVITY_NEOFORGE_NATIVE_PATH_RECOVERY_2026-07-30.json)；断线重连与多人行为仍随灰度执行 |
| 安装失败自动恢复当前版本 | 已完成 | 原子切换与失败回滚测试 |
| 玩家主动回滚到上一版本 | 已完成 | 同锁安全克隆、玩家数据保留、确认界面、运行中保护、Java 复验及失败恢复测试 |
| Fabric 启动 | 已完成 | 基础与恐怖整蛊档案 |
| NeoForge 启动 | 已完成 | 活动档案 `21.11.42`；`0.12.2` 源码运行时和安装版真实进服的五个原生目录属性验收通过 |

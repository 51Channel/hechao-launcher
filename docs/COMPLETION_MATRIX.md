...
### 1.4 玩家主页三栏改版候选（2026-08-08）

| 项目 | 状态 | 证据 |
| --- | --- | --- |
| 宽导航与服务器目录 | 候选已完成，未发布 | 五个玩家工作区改为图标加文字的宽导航；服务器列表继续绑定真实目录、三态状态、版本、排期和人数，长任务选择门禁保持不变 |
| 当前服务器主视区 | 候选已完成，未发布 | 使用现有品牌横幅和 `SelectedServer` 派生状态；主按钮继续绑定 `PrimaryActionCommand`，没有增加假延迟、收藏或管理员操作 |
| 公告与近期活动 | 候选已完成，未发布 | 公告只投影可见服务器的非空 `Announcement`；近期活动只显示仍在进行或未来的前三个真实排期，均有明确空状态 |
| 快捷设置与维护能力 | 候选已完成，未发布 | Java、内存和游戏目录使用原设置与命令；完整安装进度、修复、回滚、删除和自定义 Java 继续位于下方可滚动区域 |
| 自动与视觉验证 | 候选已完成，未发布 | Release 构建 `0` 警告、`0` 错误；启动器 `222/222`、完整解决方案 `707/707`；`1500 x 860` 与 `1060 x 640` WPF 截图无横向溢出、重叠或操作裁切；Impeccable detector 无发现 |

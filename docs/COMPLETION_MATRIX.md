...
| 恐怖整蛊跨版本返回大厅授权 | 首条隔离实机会话与反向拒绝已通过 | 迁移 018、目标服级后台开关和授权规则已实现；API `0.21.0` 已在生产数据库副本上验证默认关闭、只开启大厅后恐怖整蛊回程允许、反向仍拒绝且模组档案保护不能绕过。旧隔离 Velocity `3.4.0-SNAPSHOT` / `3.5.1` 分别因多余 `131` / `16` 字节失败；当前 Velocity `4.0.0` build `6` + Java 25 已改为代理 Via `0`、大厅 Via `2` 的单层翻译。真实恐怖整蛊 -> `/hub` -> Lobby 首条会话稳定超过 `591` 秒，四段日志解码错误 `0`，身份、聊天、LuckPerms、TPS/MSPT 与 GC 通过；大厅反向请求 `pvp` 被真实不兼容策略拒绝，目标后端连接 `0`、玩家保持在线，随后启动器退出码 `0`。生产 Velocity、API、游戏服和真正 PVP 均未改动。仍待目视内容确认、第二次进入和成功重连；证据见 `API_PROTOCOL_TRANSLATION_CANDIDATE_2026-07-28.json`、`PVP_RETURN_PROTOCOL_STAGING_2026-07-28.json`、`PVP_RETURN_REAL_SESSION_2026-07-28.json` 与 `VELOCITY_AUTHORIZER_RELEASE_0.3.1_CANDIDATE.md` |
| NPC、`/hub`、断线重连和 API 故障路径 | 外部验收 | 前两次跨版本 `/hub` 分别在 Velocity `3.4.0-SNAPSHOT` 和 `3.5.1` 发生协议状态解码错误；修正为 Velocity 4 后端单层 Via 后，首条真实恐怖整蛊 -> `/hub` -> Lobby、反向不兼容拒绝和正常退出均通过。第二枚 fresh grant 已签发，仍需玩家点击进入以完成再次进入、成功重连和第二次退出；NPC 与其余路径需要真实玩家和真实代理日志 |

...
| 项目 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| 启动器 | 已发布 | `0.14.0` 已发布到私有 OSS；匿名读取 `403`、两轮签名回读 `200`、长度与 SHA-256 一致，管理机已完成 `0.13.6 -> 0.14.0` 引导升级并保留会话与设置，见 [`LAUNCHER_RELEASE_0.14.0.md`](LAUNCHER_RELEASE_0.14.0.md) |
...
| Windows 安装、覆盖升级与卸载 | 已完成 | 保留既有验收链；本次完成 `0.13.7 -> 0.14.0` 隔离覆盖升级、全新安装、双轮卸载、设置与会话保留，并完成管理机 `0.13.6 -> 0.14.0` 引导升级 |
| 私有 OSS 发布 | 已完成 | `0.14.0` 不可变对象已发布；第二轮重复发布校验后跳过，匿名读取 `403`，签名回读 `200`，私有签名 URL 未进入文档或日志 |
| 自动测试 | 已完成 | .NET `480/480`，Velocity `26/26`，Lobby `3/3`，LuckPerms `4/4`，Paper/Purpur `2/2`，授权切换闸门 `4/4` |
| 2 至 3 人真实灰度 | 外部验收 | 待按 `0.14.0` 单进程切服与 Lobby 隔离清单执行 |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| 正式版本与制品 | 已完成 | `0.14.0` 已发布私有 OSS；EXE 与安装包版本、大小、SHA-256、`NotSigned` 状态见 [`LAUNCHER_RELEASE_0.14.0.md`](LAUNCHER_RELEASE_0.14.0.md) |
| 自动测试 | 已完成 | 完整解决方案 `492/492`，启动器 `143/143` |
...
| 生产更新通道 | 已完成 | `LatestVersion=0.14.0`，公网健康端点为 `200`，服务 `NRestarts=0` |
| `0.13.5` 中间构建 | 已归档 | 仅内部验收，不创建标签、不作为回滚目标、不覆盖其不可变 OSS 对象 |

结构化验收见
[`evidence/LAUNCHER_0.14.0_RELEASE_ACCEPTANCE_2026-07-31.json`](evidence/LAUNCHER_0.14.0_RELEASE_ACCEPTANCE_2026-07-31.json)。

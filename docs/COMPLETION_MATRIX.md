...
| 项目 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| 启动器 | 已发布，待本机自动更新验收 | `0.14.1` 已发布到私有 OSS；匿名读取 `403`、两轮签名回读 `200`、长度与 SHA-256 一致；管理机上的 `0.14.0` 进程保持不变，等待用户亲自重开验证自动升级，见 [`LAUNCHER_RELEASE_0.14.1.md`](LAUNCHER_RELEASE_0.14.1.md) |
...
| Windows 安装、覆盖升级与卸载 | 已完成 | 保留既有验收链；本次完成 `0.14.0 -> 0.14.1` 隔离覆盖升级、全新安装、双轮卸载、设置与会话保留；管理机保持 `0.14.0`，等待用户重开验收自动升级 |
| 私有 OSS 发布 | 已完成 | `0.14.1` 不可变对象已发布；第二轮重复发布校验后跳过，匿名读取 `403`，两轮签名回读 `200`，私有签名 URL 未进入文档或日志 |
| 自动测试 | 已完成 | .NET `501/501`，Velocity `26/26`，Lobby `3/3`，LuckPerms `4/4`，Paper/Purpur `2/2`，授权切换闸门 `4/4` |
| 2 至 3 人真实灰度 | 外部验收 | 待按 `0.14.1` 单进程切服与 Lobby 隔离清单执行 |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| 正式版本与制品 | 已完成 | `0.14.1` 已发布私有 OSS；EXE 与安装包版本、大小、SHA-256、`NotSigned` 状态见 [`LAUNCHER_RELEASE_0.14.1.md`](LAUNCHER_RELEASE_0.14.1.md) |
| 自动测试 | 已完成 | 完整解决方案 `501/501`，启动器 `144/144` |
...
| 生产更新通道 | 已完成 | `LatestVersion=0.14.1`，公网健康与就绪端点均为 `200`，服务 `NRestarts=0` |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| 页面布局与信息层级 | 候选已完成，未发布 | 常规 `1500 x 860` 与最小 `1060 x 640` 窗口已检查服务器、活动、下载、账户和设置页，无裁字、重叠或操作区溢出；账户页与左下角统一显示玩家皮肤头像 |
| 键盘与状态可访问性 | 候选已完成，未发布 | 全局遮罩焦点、Toast live region、非颜色安装阶段、焦点环、开关自动化名称和长路径 Tooltip 均有契约测试 |
| 安装、删除、启动和目录切换一致性 | 候选已完成，未发布 | 长任务冻结服务器、档案、目录、Java 与内存上下文；generation、取消令牌、目录快照和每请求来源阻止旧响应覆盖新目录 |
| 活动目录和排期 | 候选已完成，未发布 | 显式 `ServerCatalogSection`、旧缓存兼容、本地时间排期、分段长延时、边界刷新及进服前权威目录复查均有测试；刷新取消时故障关闭 |
| 玩家设置与真实运行目录 | 候选已完成，未发布 | 运行状态携带真实 `DataRoot`；切服前和退出后以实际运行目录同步 `options.txt`；运行状态与共享设置均有规范路径文件锁、超时和并发恢复测试 |
| 账号错误恢复 | 候选已完成，未发布 | 注册已完成但自动登录失败时明确引导登录；异常响应不会从 WPF 点击事件逸出，忙碌态可恢复 |
| 自更新失败恢复与最低版本门禁 | 候选已完成，未发布 | 同一失败版本不会形成启动重启循环，保留当前版本和手动重试入口；强制更新同时门禁命令与主操作方法 |
| 自动验证 | 候选已完成，未发布 | 完整解决方案 `564/564`，启动器 `202/202`；详见 [`LAUNCHER_FRONTEND_AUDIT_2026-08-01.md`](LAUNCHER_FRONTEND_AUDIT_2026-08-01.md) |
...
| 启动器本体自更新 | 已完成，真实升级待最终验收 | 每个进程启动并恢复登录态后检查；私有 OSS 断点下载、长度与 SHA-256 复验、临时更新器静默覆盖、失败保留及重试入口均已发布。生产 `LatestVersion=0.14.1`；本次候选进一步阻止同一失败版本形成重启循环，见 [`LAUNCHER_SELF_UPDATE_OPERATIONS.md`](LAUNCHER_SELF_UPDATE_OPERATIONS.md) |
...
9. [目录切换与自动恢复工具已完成] 用 enforce 五人证据启用目录强制登录，继续完成
   `2/3/5/20` 人灰度及真实回滚。
10. [`0.14.1` 已发布，真实自动升级待用户重开验收] 本体自更新和真实 Minecraft
    皮肤头像已进入生产通道；仍需完成一次旧安装版到 `0.14.1` 的真实自动升级验收。
11. [生产只读验收已完成] 服控 API、owl5 与 owl9 代理已启用；9 个目标、2 个代理、
    4 个运行实例与冲突组均已核对，部署前后游戏 PID 不变。下一步只在专用无玩家
    目标完成真实启停、失败取消、终端和快捷设置验收。

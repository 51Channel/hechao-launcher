...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| 页面布局与信息层级 | 候选已完成，未发布 | 常规 `1500 x 860` 与最小 `1060 x 640` 窗口已检查服务器、活动、下载、账户和设置页，无裁字、重叠或操作区溢出 |
| 键盘与状态可访问性 | 候选已完成，未发布 | 全局遮罩焦点、Toast live region、非颜色安装阶段、焦点环、开关自动化名称和长路径 Tooltip 均有契约测试 |
| 安装、删除、启动和目录切换一致性 | 候选已完成，未发布 | 长任务冻结服务器、档案、目录、Java 与内存上下文；generation、取消令牌和目录快照阻止旧响应覆盖新目录 |

...
> 更新时间：`2026-08-09`
>
> 当前生产：启动器 `0.15.3`、API `0.29.0`、Publisher Agent `1.2.1`、
...
| 项目 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| 启动器 | 已发布，更新通道已开放 | `0.15.3` 已发布到私有 OSS；两轮签名回读 `200`、匿名读取 `403`，真实登录会话确认 `0.15.2` 可取得更新计划并完整下载，见 [`LAUNCHER_RELEASE_0.15.3.md`](LAUNCHER_RELEASE_0.15.3.md) |
...
| Windows 安装、覆盖升级与卸载 | 已完成 | `0.15.2 -> 0.15.3` 隔离覆盖升级、全新安装、双轮卸载、设置与会话保留均通过；既有正式启动器进程未被替代关闭 |
| 私有 OSS 发布 | 已完成 | `0.15.3` 不可变对象已发布；第二轮重复发布校验后跳过，匿名读取 `403`，两轮签名回读 `200`，私有签名 URL 未进入 Git、文档或终端记录 |
| 自动测试 | 已完成 | `0.15.3` 完整解决方案 `710/710`、Launcher `225/225`、Publisher `55/55`；API `0.29.0` 为 API `294/294`、完整解决方案 `700/700`、Vitest `8/8`、Playwright `20/20` |
| 2 至 3 人真实灰度 | 外部验收 | 待按 `0.15.3` 单进程切服与 Lobby 隔离清单执行 |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| 正式版本与制品 | 已完成 | `0.15.3` 已发布私有 OSS；EXE 与安装包版本、大小、SHA-256、`NotSigned` 状态见 [`LAUNCHER_RELEASE_0.15.3.md`](LAUNCHER_RELEASE_0.15.3.md) |
...
| 服务器主页三栏改版 | 已完成 | 宽导航、真实服务器目录、导航栏账号区、连续当前服务器主卡片、真实公告、近期活动、三点维护菜单和当前档案快捷设置已上线；未引入伪造数据或玩家端管理入口 |
...
| 生产更新通道 | 已完成 | `LatestVersion=0.15.3`、`MinimumSupportedVersion=0.12.3`；真实会话与 API 签发下载链通过，公网健康与就绪端点均为 `200`，服务 `NRestarts=0`；切换前环境备份与禁用更新回滚路径均保留 |
...
| 自动与视觉验证 | 已发布（`0.15.3`） | Release 构建 `0` 警告、`0` 错误；启动器 `225/225`、完整解决方案 `710/710`；`1500 x 860` 与 `1060 x 640` WPF 截图无横向溢出、重叠或操作裁切；Impeccable detector 无发现 |
...
| 启动器本体自更新 | 已完成 | 每个进程启动并恢复登录态后检查；私有 OSS 断点下载、长度与 SHA-256 复验、临时更新器静默覆盖、失败保留及重试入口均已发布。生产 `LatestVersion=0.15.3`；真实会话确认 `0.15.2` 可更新且 `0.15.3` 不重复提示，见 [`LAUNCHER_SELF_UPDATE_OPERATIONS.md`](LAUNCHER_SELF_UPDATE_OPERATIONS.md) |

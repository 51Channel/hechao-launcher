...
| 下载量、失败率与版本分布 | 已实现待生产验收 | API `0.20.2` 保留迁移 15、30 天留存、幂等批次和后台“运行数据”，并对客户端下载失败率建立统一告警；真实基础档案启动已完成，仍需 Activity/PVP 安装、修复、回滚和多人样本 |
...
| 能力 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| Velocity 异步授权、首次目标与客户端兼容 | 已实现待生产验收 | API `0.20.2` + 插件 `0.3.0` 以 `monitor` 运行；生产兼容矩阵 `8/8`，基础档案 Lobby/Survival1/Survival2 真实转服通过，不兼容 Activity/PVP 会立即拒绝；正确模组档案真实进服仍待验收 |

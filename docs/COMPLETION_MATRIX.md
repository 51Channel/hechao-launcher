...
> 更新时间：`2026-07-30`
>
> 当前生产：启动器 `0.12.0`、API `0.22.0-20260729T144953Z`、
...
| 项目 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| 启动器 `0.12.0` | 已部署待外部验收 | 私有 OSS 已发布；匿名 `403`、两次签名回读 `200`、长度和 SHA-256 复验通过，见 [`LAUNCHER_RELEASE_0.12.0.md`](LAUNCHER_RELEASE_0.12.0.md) |
...
| Windows 安装、覆盖升级与卸载 | 已完成 | `0.11.16 -> 0.12.0`、隔离干净安装、双轮卸载、设置与 DPAPI 会话保留、ProductVersion 和运行中启动器保护均通过 |
| 私有 OSS 发布 | 已完成 | `0.12.0` 不可变对象已发布；重复发布校验后跳过，私有签名 URL 未进入文档或日志 |
...
| 启动器唯一切服与单 Minecraft 进程 | 已部署待外部验收 | `0.12.0` 已发布并实现持久进程接管、安全退出/超时收口、退出完成后才重新授权、全局单进程和全部 Lobby 目录过滤；仍需同档案/跨档案各三轮真实切换与启动器重启接管 |
...
| 下载量、失败率与版本分布 | 已实现待生产验收 | API `0.22.0` 保留迁移 15、30 天留存、幂等批次和后台“运行数据”，并对客户端下载失败率建立统一告警；真实基础、Activity 与恐怖整蛊安装样本已完成，仍需 Activity/恐怖整蛊修复、回滚、完整 Launch/GameExit 和多人样本 |

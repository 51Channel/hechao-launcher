...
| 天域长期生存服一键包 `1.0.9` | 结构验证通过，禁止生产部署 | 标准包结构为 `Canonical`，客户端根路径、启动档案、Java `21` 元数据和 `survival2:25565` 满足导入合同；但 2026-08-16 复核确认屏幕服主商品按钮路由失效，且迁移 `029` 会拒绝非 `minecraft:` 的模组商品 ID。需升级 HechaoEconomy/Screen 至至少 `0.1.3`、生成高于 `1.0.9` 的新包并重跑全量验证；当前包未上传 OSS、未部署或开服，见 [`SKYREALM_ECONOMY_AND_SCREEN_DEVELOPMENT_HANDOFF.md`](SKYREALM_ECONOMY_AND_SCREEN_DEVELOPMENT_HANDOFF.md) |

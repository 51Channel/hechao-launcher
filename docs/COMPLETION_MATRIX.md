...
| Microsoft/Minecraft 正版绑定 | 已实现待生产验收 | API 许可已通过；当前 22 个社区账号中只有 1 个绑定 Minecraft，平台等级为 21 个 Member、1 个 Administrator，Participant 与 Collaborator 均为 0，仍需四级真实账号验收 |
...
| 下载量、失败率与版本分布 | 已实现待生产验收 | API `0.20.1` 保留迁移 15、30 天留存、幂等批次和后台“运行数据”，并对客户端下载失败率建立统一告警；生产当前只有 1 条 `LauncherStarted/Success`，Install、Repair、Rollback、Launch 与 GameExit 成功样本均为 0，仍需真实操作和后台页面验收 |
...
| TPS、MSPT、进程内存和启动时间 | 已实现待生产验收 | API `0.19.0` 与采集器 `0.2.0` 已生产上线，进程内存、CPU、启动时间和磁盘已实时入库；PVP 停服时也由 `owl9` 上报磁盘与固定问题代码。Paper/Purpur 代理已部署到大厅、Survival1、Survival2；NeoForge 1.21.11 与 Fabric 1.20.1 代理也已分别静态部署到活动服和 PVP，全部等待服主下次自行启动/重启后加载并验证 TPS/MSPT/GC |
| 管理后台状态与错误摘要 | 已实现待生产验收 | “服务状态”和“运行告警”页面、30 天样本、24 小时固定问题摘要与告警历史保留在 API `0.20.1`；真实管理员 MFA 已登记，但当前有效后台会话为 0。匿名数据库核对显示 5 条预期活跃告警，仍待从启动器创建新票据并逐页验收 |

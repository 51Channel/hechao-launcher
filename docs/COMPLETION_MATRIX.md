...
>
> Velocity 授权插件：`0.3.1`，当前模式 `monitor`；客户端不兼容硬拒绝
...
| 启动器唯一切服与单 Minecraft 进程 | 进行中 | 需要实现当前游戏安全退出、超时收口、新目标重新授权、同档案/跨档案三轮切换，并确保玩家在线目录、缓存和内置目录均无大厅 |
...
| 能力 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| Velocity 异步授权、首次目标与客户端兼容 | 已实现待生产验收 | 生产代理已迁移到 Velocity 4、Java 25 与 Authorizer `0.3.1` monitor；兼容矩阵 `8/8`，Survival1/Survival2、Activity 与恐怖整蛊的单账号直接目标已有样本。当前 monitor 在授权 API 故障时仍会保留原始大厅目标；必须改成首次连接无有效授权即硬拒绝，并复测四级账号、重新授权、重连和故障路径 |
...
| NPC、`/hub`、`/lobby` 和游戏内转服 | 已取消，待生产移除 | 玩家只能通过启动器切服；需要从 Velocity 和各后端移除命令、NPC 与默认回退，并验证没有可用旁路 |
| 基础设施大厅玩家隔离 | 进行中 | 保留大厅任务、`25566` 内部监听、LuckPerms 等级代理、指标、告警和备份；大厅只可作为首次改写的内部引导占位，不可授权或回退。需要增加独立后端拒绝玩家保护，并从玩家目录、Via 回程、命令和 NPC 中移除 |
| Velocity `enforce` | 外部验收 | 四级账号与全部转服路径通过后切换 |
| 目录强制登录 | 外部验收 | `enforce` 稳定后启用 `Authentication__EnforceCatalogAuthentication=true` |
| 在线、人数、协议和软件版本心跳 | 已完成，待大厅角色调整 | `owl5` 四目标与 owl9 恐怖整蛊的历史 `owl9-pvp` 单目标只读采集器每分钟上报。当前告警查询只纳入 `is_visible` 目录目标；需要拆分玩家可见性与 `monitoring_enabled`，让大厅继续告警但不计入玩家可选目标或开放率。真正 PVP 未接入该心跳 |
...
7. 完成启动器唯一切服，以可回滚变更集隔离大厅玩家入口并移除 HubCommand、Via 回程和游戏内转服；大厅任务、LuckPerms 等级代理、指标和备份保持在线。
8. 复测所有目标的直接进入、同档案/跨档案三轮切换、断线恢复与 API 故障，再切换 Velocity `enforce`。

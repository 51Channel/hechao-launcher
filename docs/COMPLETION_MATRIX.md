...
> 更新时间：`2026-08-14`
>
> 当前生产：启动器 `0.15.7`、API `0.30.7`、LuckPerms Tier Agent `0.1.3`、Publisher Agent `1.2.1`、
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| 正式版本与制品 | 已完成 | `0.15.7` 已发布私有 OSS；EXE 与安装包版本、大小、SHA-256、`NotSigned` 状态见 [`LAUNCHER_RELEASE_0.15.7.md`](LAUNCHER_RELEASE_0.15.7.md) |
| 自动测试 | 已完成 | 完整解决方案 `721/721`，启动器 `227/227`，API `302/302`，Publisher `55/55`，管理后台 Vitest `11/11` |
| 官网同源活动月历 | 已完成 | Launcher 后台、官网后台、Launcher 活动页和官网公开日历读取同一 API 企划；Launcher 活动页进入即刷新并每 30 秒同步，跨午夜排期按 `[开始, 结束)` 计算，零活动时仍显示完整月历 |
...
| 生产更新通道 | 已部署待会话复验 | `LatestVersion=0.15.7`、`MinimumSupportedVersion=0.12.3`；公网健康、就绪、公开元数据和官网下载网关通过，服务 `NRestarts=0`；当前管理机无可恢复 Launcher 会话，认证端点复验待登录后补跑 |

...
| 自动测试 | 已完成 | 当前开发分支 .NET `368/368`，Velocity `13/13`，LuckPerms 等级代理 `4/4`，Paper/Purpur 指标代理 `2/2`；日志配置恢复、私有重定向日志边界、客户端兼容判定、协议转换默认关闭及 RAM 最小权限断言均已覆盖 |
...
| PVP 跨版本返回大厅授权 | 已实现待隔离实机验收 | 迁移 018、目标服级后台开关和授权规则已实现，默认关闭且不会绕过模组档案；回环隔离代理已用协议 `763/774` 完成 status 协商且 Via 错误为 `0`，仍待真实登录与 `/hub`；现网 Via JAR 仍禁用，生产行为未变化，证据见 `PVP_RETURN_PROTOCOL_STAGING_2026-07-28.json` |

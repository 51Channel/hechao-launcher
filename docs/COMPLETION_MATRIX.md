...
| API `0.31.0` / Agent `0.6.0` 候选 | 已实现待生产验收 | 新增权威 Economy Service，并把整合包部署从单一活动槽扩展到 Agent 显式授权的合法目标；`survival2` 候选配置已加入，活动企划仍严格固定 `activity`。未部署 API、迁移或 Agent，见 [`API_RELEASE_0.31.0_CANDIDATE.md`](API_RELEASE_0.31.0_CANDIDATE.md) 与 [`SERVER_CONTROL_AGENT_RELEASE_0.6.0_CANDIDATE.md`](SERVER_CONTROL_AGENT_RELEASE_0.6.0_CANDIDATE.md) |
...
| 天域长期生存服一键包 `1.0.3` | 已实现待生产验收 | 标准包 `Canonical`、零阻断，包含权威经济插件与 NeoForge 双端屏幕；后台多目标时强制选择，部署确认包含 `survival2`。包级逐文件/JAR 资源校验通过，未上传 OSS、未部署或开服，见 [`SKYREALM_ECONOMY_INTEGRATION_1.0.3.md`](SKYREALM_ECONOMY_INTEGRATION_1.0.3.md) |

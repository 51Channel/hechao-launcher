...
> 更新时间：`2026-08-06`
>
> 当前生产：启动器 `0.14.2`、API `0.28.2`、Publisher Agent `1.0.0`、
> owl5 ServerControlAgent `0.4.0`、owl9 ServerControlAgent `0.4.0`、
...
| API `0.28.2` | 已完成 | Vue 十页、迁移 24/24、整合包任务编排和白名单服务端文件删除均已部署；精确确认文本在 3 秒轮询期间保持，已完成删除和清理的目标从日常服控列表隐藏。数据库与配置备份、原子切换、内外网健康和生产状态回归通过，见 [`API_RELEASE_0.28.2.md`](API_RELEASE_0.28.2.md) |
...
| 自动测试 | 已完成 | 当前基线为完整解决方案 `670/670`、API `282/282`、Publisher `39/39`、ServerControlAgent `51/51`、Vue Vitest `8/8`、Playwright `16/16`；Velocity、Lobby、LuckPerms 和指标代理保持各自既有正式基线 |
...
| 项目 | 状态 | 证据 |
| --- | --- | --- |
| API 与代理版本 | 已完成 | API `0.28.2`、owl5 与 owl9 代理 `0.4.0` 已部署；API 服务正常，代理在线状态和删除能力按实时心跳故障关闭，见 [`API_RELEASE_0.28.2.md`](API_RELEASE_0.28.2.md) 与 [`SERVER_CONTROL_AGENT_RELEASE_0.4.0.md`](SERVER_CONTROL_AGENT_RELEASE_0.4.0.md) |
| 目录与物理服状态同步 | 已完成 | `Online` 只作为管理员开放策略；同名服控目标在线时开放、停止时自动关闭、服控失联时故障关闭。目录删除并完成清理后，目标从服控概览隐藏；重新部署目录并恢复心跳后自动出现。`dollnight` 显式维护保持优先 |
| 代理心跳与命令隔离 | 已完成 | 心跳和命令使用独立循环；两台 `0.4.0` 代理保留循环自恢复，并用目标级门闩隔离目录心跳、快捷设置、整合包切换和文件删除。升级前后 owl5 五个、owl9 一个 Java PID 的身份不变 |
...
| 生产进程隔离 | 已完成 | `0.28.2` 只重启 API，没有向 Minecraft、Velocity、Publisher 或两台服控代理发送控制命令。生产已有 `activity/fanstreet/yugong` 三个成功删除记录；三者目录不存在、清理完成且无活动操作，当前概览显示其余 6 个目标 |
...
| 下载量、失败率与版本分布 | 已实现待生产验收 | 当前 API `0.28.2` 保留迁移 15、30 天留存、幂等批次和后台“运行数据”，并对客户端下载失败率建立统一告警；真实基础、Activity 与恐怖整蛊安装样本已完成，仍需真实回滚、完整 Launch/GameExit 和多人样本 |
...
| 账号、设备会话与 Minecraft UUID 封禁 | 已实现待生产验收 | 功能自 API `0.16.0` 上线并保留在当前 `0.28.2`；账号停用/恢复、单设备和全部会话撤销、UUID 定时封禁、论坛既有 Cookie 联动、并发保护、Velocity/目录拒绝及审计均通过隔离与生产链路验收。真实管理员 MFA 已登记，仍待在生产页面逐项操作验收 |
...
| 删除停用服务端文件 | 已完成 | API `0.28.2` 与两台代理 `0.4.0` 已生产部署；后台危险确认、CSRF、原因、审计、停止态、命令互斥、路径边界、同卷移出、清理重试和幂等均已验收。生产已有 `activity/fanstreet/yugong` 三个成功删除记录；完成清理后不再显示在日常服控列表，审计、代理配置和外置备份保留，重新部署后自动恢复。见 [`API_RELEASE_0.28.1.md`](API_RELEASE_0.28.1.md) |
...
11. [生产只读验收已完成] 服控 API `0.28.2`、owl5 与 owl9 代理 `0.4.0` 已启用；
    数据库保留 9 个目标，服控概览当前显示 6 个现存目录目标，3 个成功删除目标保留审计但不占用列表。

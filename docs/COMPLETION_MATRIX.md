...
| 自动测试 | 已完成 | .NET `360/360`，Velocity `13/13`，LuckPerms 等级代理 `4/4`，Paper/Purpur 指标代理 `2/2`；私有重定向日志边界、客户端兼容判定及 RAM 最小权限断言均已覆盖 |
...
| Activity/PVP 生产档案安装 | 已完成 | 正确赫朝启动器已安装 Activity `1.0.10` 与 PVP `1.0.0`，独立 Java 21/17 和目录均存在；清单 `8,503` 项无缺失，`8,501` 项逐字节一致，另两项仅为客户端运行时可变配置。真实路由、修复和回滚仍按独立生产门槛验收 |
...
| 能力 | 状态 | 证据或剩余条件 |
| --- | --- | --- |
| Velocity 异步授权、首次目标与客户端兼容 | 已实现待生产验收 | API `0.20.2` + 插件 `0.3.0` 以 `monitor` 运行；生产兼容矩阵 `8/8`，基础档案 Lobby/Survival1/Survival2 真实转服通过，不兼容 Activity/PVP 会立即拒绝；正确模组档案已完成生产安装，真实进服仍待验收 |

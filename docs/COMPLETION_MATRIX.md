...
| 恐怖整蛊跨版本返回大厅授权 | 已实现待隔离实机复测 | 迁移 018、目标服级后台开关和授权规则已实现；API `0.21.0` 已在生产数据库副本上验证全部既有目标默认关闭、只开启大厅后恐怖整蛊回程允许、反向仍拒绝且 NeoForge 档案保护不能绕过。旧隔离 Velocity `3.4.0-SNAPSHOT` 的第一次 `/hub` 在后端切换时触发 `accept_teleportation` 多余 `131` 字节；Velocity `3.5.1` build `615` 实机复测能够进入 Lobby，但仍因多余 `16` 字节断开。隔离核心现为稳定版 Velocity `4.0.0` build `6` 和独立 Temurin Java `25.0.4+7`；核对后发现代理与大厅重复加载 Via，已改为代理 Via `0`、大厅 Via `2` 的单层翻译，三个代理插件、协议 `393/763/774` 与错误日志 `0` 均通过。生产 Velocity、API、游戏服和真正 PVP 均未改动。仍待单层翻译候选的真实 `/hub`、权限与重连复测；证据见 `API_PROTOCOL_TRANSLATION_CANDIDATE_2026-07-28.json`、`PVP_RETURN_PROTOCOL_STAGING_2026-07-28.json`、`PVP_RETURN_REAL_SESSION_2026-07-28.json` 与 `VELOCITY_AUTHORIZER_RELEASE_0.3.1_CANDIDATE.md` |
| NPC、`/hub`、断线重连和 API 故障路径 | 外部验收 | 一次恐怖整蛊重连在 Velocity 认证前超时，仍需成功样本。前两次跨版本 `/hub` 已证明命令、授权和大厅可达，但分别在 Velocity `3.4.0-SNAPSHOT` 和 `3.5.1` 发生协议状态解码错误；Velocity 4 隔离候选已就绪，仍需完成真实回大厅、权限核对、再次进入和正常退出。NPC 与其余路径需要真实玩家和真实代理日志 |
...
7. 实现恐怖整蛊专用回程，验证兼容 NPC/命令转服、断线重连和 API 故障，再切换 Velocity `enforce`。

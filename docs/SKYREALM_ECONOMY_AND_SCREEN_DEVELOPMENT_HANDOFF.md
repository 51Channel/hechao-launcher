...
下表作为修复版的首发配置基线。价格单位沿用当前实现中的“金币”，但全部使用整数，后续
把显示名称改为“赫币”时不需要换算余额精度。`1.0.9` 数据库当前仍为空，这张表尚未写入
迁移或生产数据库。
...
- 一名玩家把 17 项个人额度全部卖满时，理论最高收入为 `5,136`，接近候选设计的
  每日约 `5,000` 金币校准目标。当前服务尚无跨商品的总收入硬上限，所以通过逐项额度
  近似控制；后续仍应增加账户级每日系统回收总额门禁。
- 全服日限统一按 `20 x 个人日限` 设置，允许 20 名灰度玩家各自达到上限；超过 20 人时
  全服额度会先到先得，防止大规模自动化持续铸币。
- 当前代码按 UTC 日期记录额度，因此北京时间每天 `08:00` 进入新额度日，不是零点。
- 原矿只回收粗矿形态，不同时回收铁锭、铜锭、金锭、矿块和粒，避免熔炼、拆块或农场
  产物形成重复计价路径。
- 绿宝石首发禁售，避免村民交易循环；铁锭、金锭和铜锭首发禁售，避免铁傀儡、猪灵塔、
  溺尸塔等自动化来源直接铸币。
- 圆石、泥土、木材、竹子、甘蔗、仙人掌、南瓜和西瓜首发禁售，避免高吞吐基础农场
  占满系统回收预算。
- 末影珍珠、黏液球、海晶材料和其他可集中刷取的掉落物首发禁售；后续根据 7 天产量数据
  决定是否加入。
- 所有非 `minecraft:` 命名空间物品首发禁售。Create、Sable 和其他模组材料必须在完成
  配方闭环、单位时间产量和复制路径审计后，另建“模组商品表 v2”。

修复版可以通过受审计的商品导入或服主持物命令写入上述配置。手工配置时使用
`/heco product set <单价> <个人日限> <全服日限>`。部署后，玩家执行 `/shop` 看到的
启用目录才是当时真正可以出售的权威清单；文档表格不能覆盖数据库实时状态。

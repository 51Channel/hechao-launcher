...
> 启动器源码版本：`0.12.2`
> 发布器源码版本：`0.9.0`
> 当前状态：私有 OSS Bucket、下载域名 CNAME/HTTPS、读写分离 RAM 身份、本地鉴权下载链和生产签名信任链均已完成；基础、Vanilla、Forge、NeoForge 活动、恐怖整蛊 Fabric（历史档案 ID `pvp-fabric-1.20.1`）与 DollNight 六份档案已由 API `0.22.0` 托管，启动器 `0.12.2` 已完成退出状态刷新、日志配置恢复、启动检查控制、隐私受限运行遥测、NeoForge 原生目录规范化与私有 OSS 发布闭环。
>

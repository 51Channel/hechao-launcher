...
> 启动器源码版本：`0.11.3`
> 发布器源码版本：`0.8.1`
> 当前状态：私有 OSS Bucket、下载域名 CNAME/HTTPS、读写分离 RAM 身份、本地鉴权下载链、生产签名信任链，以及基础与 NeoForge 活动档案的 API `0.11.1` 在线激活均已完成；启动器 `0.11.3` 已通过本机升级、平滑并行进度、全量落盘、Java 兼容路径与页面边界验证，最终安装包尚未上传。

...
2026-08-16 只读审查确认，工业季 HechaoEconomy `0.1.2` 与双端 Economy Screen
`0.1.2` 已部署，但 Economy API、账本迁移和外置凭据尚未上线。生产插件因此保持故障
关闭，TAB 金币为 `--`，所有写交易不可用且没有回退到 Essentials 本地余额。

旧经济候选分支使用迁移 `029_economy_ledger`，与现行 `029/030` 独立槽迁移冲突，
不得直接部署；必须基于最新主线使用新的加法迁移和高于 `0.32.2` 的 API 版本重新集成。
占位符缓存预热、Essentials 命令所有权、Vault 只读兼容、屏幕会话重放、第三方服务器
界面伪装和低分辨率布局也必须在下一版修复。完整证据和施工顺序见
[`SKYREALM_ECONOMY_AND_SCREEN_AUDIT_2026-08-16.md`](SKYREALM_ECONOMY_AND_SCREEN_AUDIT_2026-08-16.md)。

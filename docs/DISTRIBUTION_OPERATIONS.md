...
客户端档案的 `DownloadBytes` 是完整档案大小，用于本地 SHA-256 检查、磁盘规划和完整性
校验，不等于每次更新的网络下载量。安装器按以下顺序处理每个清单文件：

1. 校验当前活动目录，内容与清单一致时直接复制或硬链接到暂存目录；
2. 当前文件不一致时校验按 SHA-256 寻址的对象缓存，缓存有效时直接复用；
3. 只有当前文件和对象缓存都不可用时，才把唯一对象加入网络下载队列；
4. 所有对象校验通过后原子切换活动目录，失败时保留原有可用版本。

启动器 `0.15.9` 起，下载页把“检查本地文件”“增量下载”“准备客户端”“切换版本”
和“准备配套 Java”分开显示。只有增量下载阶段显示网络速度；该阶段的总字节按缺失的
唯一对象求和，多个目标路径引用同一 SHA-256 时只计算一次。工业季 `1.0.11 -> 1.0.12`
实际只新增一个 `27,450` 字节的 JAR，完整 `1.12 GB` 仅代表本地需要检查的档案规模。
候选与发布记录见 [`LAUNCHER_RELEASE_0.15.9_CANDIDATE.md`](LAUNCHER_RELEASE_0.15.9_CANDIDATE.md)。
>
...
`2026-08-09` 已将启动器 `0.15.4` 写入
`releases/launcher/0.15.4/Hechao-Launcher-Setup-0.15.4-win-x64.exe`。
首次上传后再次执行同版本发布，远端长度、版本、文件名与 SHA-256 匹配并跳过，
没有覆盖或重复上传。匿名访问返回 `403`，两次签名回读均返回 `200`；完整下载为
`61,965,169` 字节，SHA-256 为
`42D60ED9149D4ED03A9B0958482DEF9FE004C212CE95BA39912E366A665194B5`。
生产更新通道为 `LatestVersion=0.15.4`、`MinimumSupportedVersion=0.12.3`；真实
DPAPI 会话确认 `0.15.3` 生成更新计划、`0.15.4` 不重复更新。签名 URL 未进入终端、
Git 或文档；详细记录见 [`LAUNCHER_RELEASE_0.15.4.md`](LAUNCHER_RELEASE_0.15.4.md)。

`2026-08-10` 已将启动器 `0.15.5` 写入
`releases/launcher/0.15.5/Hechao-Launcher-Setup-0.15.5-win-x64.exe`。
首次上传后再次执行同版本发布，远端长度、版本、文件名与 SHA-256 匹配并跳过，
没有覆盖或重复上传。匿名访问返回 `403`，两次签名回读均返回 `200`；完整下载为
`61,963,099` 字节，SHA-256 为
`03724C0D97A32270103012426E891A55F088AE4470796C9415BB609E828DC5AF`。
生产更新通道为 `LatestVersion=0.15.5`、`MinimumSupportedVersion=0.12.3`；真实
DPAPI 会话确认 `0.15.4` 生成更新计划、`0.15.5` 不重复更新。第一次生产切换因验收
脚本错误要求公开端点返回内部最低版本字段而自动恢复，按真实脱敏契约修正后切换成功。
签名 URL 未进入终端、Git 或文档；详细记录见
[`LAUNCHER_RELEASE_0.15.5.md`](LAUNCHER_RELEASE_0.15.5.md)。

`2026-08-10` 已将启动器 `0.15.6` 写入
`releases/launcher/0.15.6/Hechao-Launcher-Setup-0.15.6-win-x64.exe`。
首次上传后再次执行同版本发布，远端长度、版本、文件名与 SHA-256 匹配并跳过，
没有覆盖或重复上传。匿名访问返回 `403`，两次签名回读均返回 `200`；完整下载为
`61,962,345` 字节，SHA-256 为
`00C5C21CE8ABEA2FB15DA49DFB2CD5BA267E582DE49DBF96D91EF938C41822B3`。
生产更新通道为 `LatestVersion=0.15.6`、`MinimumSupportedVersion=0.12.3`；真实
DPAPI 会话确认 `0.15.5` 生成更新计划、`0.15.6` 不重复更新。首次瞬态单元因
`PrivateTmp` 隔离 `/var/tmp` 而在进程启动前失败，没有发出 OSS 请求；移至 Publisher
受管 `/var/lib` 暂存区后成功，最终远端暂存、瞬态单元和瞬态凭据挂载均已清理。
签名 URL 未进入终端、Git 或文档；详细记录见
[`LAUNCHER_RELEASE_0.15.6.md`](LAUNCHER_RELEASE_0.15.6.md)。

`2026-08-10` 已将启动器 `0.15.7` 写入
`releases/launcher/0.15.7/Hechao-Launcher-Setup-0.15.7-win-x64.exe`。
首次上传后再次执行同版本发布，远端长度、版本、文件名与 SHA-256 匹配并跳过，
没有覆盖或重复上传。匿名访问返回 `403`，两次签名回读均返回 `200`；完整下载为
`61,965,027` 字节，SHA-256 为
`FA3542F3B9B7DFF9DE5CBC3605DC41882A834F7BD0E1AAEC432A210CDE673DB1`。
生产更新通道为 `LatestVersion=0.15.7`、`MinimumSupportedVersion=0.12.3`；API 保持
...
`2026-08-17` 已将启动器 `0.15.9` 写入
`releases/launcher/0.15.9/Hechao-Launcher-Setup-0.15.9-win-x64.exe`。
首次上传成功，第二次发布核对长度、版本、文件名和 SHA-256 后跳过，没有覆盖对象。
匿名访问返回 `403`，两次签名回读均返回 `200`；完整下载为 `61,995,538` 字节，
SHA-256 为
`88AE1869A47E313291AF0C60F8984BF14C1B5D0719291D72DDA254870487DE79`。
生产更新通道为 `LatestVersion=0.15.9`、`MinimumSupportedVersion=0.12.3`；API
`0.33.1`、公开元数据、官网下载网关和认证更新计划均通过。认证会话确认 `0.15.8`
生成更新计划、`0.15.9` 不重复更新；签名 URL 未进入 Git、文档或结构化证据。详细
记录见 [`LAUNCHER_RELEASE_0.15.9.md`](LAUNCHER_RELEASE_0.15.9.md)。
...
官网不保存安装包，也不公开 OSS 对象路径或长期签名 URL。Launcher API 提供两个匿名、
限流的只读入口：

- `GET /v1/public/launcher/latest` 返回版本、安装包字节数、SHA-256、发布时间和更新说明，
  不返回下载地址；更新通道关闭时返回 `204`。
- `GET /v1/public/launcher/download` 根据当前正式版本即时签发短期 OSS 地址，并以
  `302` 返回；更新通道关闭、配置不完整或签名失败时返回 `503`。

官网只能从同机回环地址调用这两个入口，不发送账号令牌、Cookie 或论坛桥接令牌。
站内 `/api/launcher/download` 必须以手动重定向模式读取响应，只转交绝对 HTTPS 地址，
不得代理约 62 MB 的安装包正文。公开元数据、HTML、日志、测试快照和文档都不得出现
真实签名 URL。

该接口已随 API `0.27.1-20260804T211905Z` 正式部署。发布时已验证元数据脱敏、匿名
限流、短期 `302`、OSS 私有边界与签名回读，再独立切换官网；`0.27.0` 发布目录和
安装包对象均未覆盖，验收证据没有保存签名地址。

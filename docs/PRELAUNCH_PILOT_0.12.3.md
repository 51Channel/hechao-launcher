...
> 已部署并通过基础健康检查后执行。
>
> 当前状态：自动发布、物理原生运行目录、Activity 安装版真实进服、同/跨档案三轮
...
- [x] 启动器 `0.12.3` 安装包 SHA-256 与正式发布记录一致，私有 OSS 匿名 `403`、两次签名回读 `200`。
- [x] Activity 工作目录与受管 Java 使用兼容短路径；五个原生目录属性全部指向不含格式字符、不是重解析点的物理 `native-runs` 目录。
- [x] 安装版 `0.12.3` 完成正版会话、连接 Activity、进入世界、正常退出码 `0` 和零残留进程验收。
...
- [x] 选择 Activity，自动使用受管 Java 21、独立 `.minecraft` 和物理原生库运行目录，完成进入目标服与正常退出。
...
Activity 再次出现模组发现停滞、`UnsatisfiedLinkError`、原生目录属性分歧、目录为
重解析点或路径包含 U+200C 时，立即停止该档案灰度并保留 `latest.log` 与玩家确认的
诊断包，不应改回整游戏目录 junction。

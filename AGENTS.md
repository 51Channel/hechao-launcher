...
- 固定活动替换入口仍为 owl5 回环 `127.0.0.1:25568`，Velocity 目标为 `activity`。
  只有实际占用该入口的替换服属于 `owl5-activity-slot`，同一时刻只能运行一个。
- 整合包可部署到固定 `activity`，也可由后台按用途创建 `activity-*`、`survival-*`、
  `pvp-*`、`minigame-*` 独立槽。动态槽从固定 `activity` 安全模板派生，但必须使用
  `25600-25611` 中的独立端口、槽自身的 Velocity 目标和空冲突组，因此可同时运行。
  创建后默认停止、隐藏，部署整合包前禁止启动。
- 活动企划必须绑定一个真实、可部署的承载服务器。新绑定可沿用整合包默认目标，也可
  选择固定 `activity` 或已就绪的独立槽；逻辑企划本身不得进入普通服务器目录或成为
  服控目标。发布后玩家目录只显示企划卡并隐藏对应真实槽，运行状态、部署和 Velocity
  授权全部解析到该真实槽。同一承载槽上的已发布排期互斥，不同独立槽可以并行。
- 不得覆盖既有 `survival2`、`lobby`、`pvp` 等固定目标；新用途必须创建独立槽 ID。
